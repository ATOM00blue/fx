...
const DIFF_LINES = (() => {
  const value = Number(process.env.X1_APPROVAL_PROFILE_DIFF_LINES ?? "50000");
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new Error(`invalid X1_APPROVAL_PROFILE_DIFF_LINES: ${JSON.stringify(value)}`);
  }
  return value;
})();
...
async function waitForPaneDeath(session: TmuxSession, timeoutMs: number): Promise<number | null> {
  ...
  throw new Error("timed out waiting for x1 process exit");
}

const binary = realpathSync(requiredEnv("X1_APPROVAL_PROFILE_BIN"));
const outRoot = requiredEnv("X1_APPROVAL_PROFILE_OUT");
const cycles = positiveInteger(process.env.X1_APPROVAL_PROFILE_CYCLES, DEFAULT_CYCLES);
...
try {
  session = await TmuxSession.create({
    ...
    env: {
      ...
      X1_TRACE_SCOPES: "permission,input,frame_render,terminal_diff",
      NO_COLOR: "1",
    },
    stderrPath,
    width: 120,
  ...
  assert(readFileSync(stderrPath, "utf8") === "", "x1 wrote to stderr");
  ...
  assert(exitStatus === 0, `x1 exit status was ${exitStatus}`);
  passed = true;
