...
This document describes the internal design of the native Node-API backend used by `libx1`, including its trust boundaries, lifecycle, resource limits, and security invariants. It is maintainer documentation, not part of the supported user-facing API.
...
| Public JavaScript agent implementation | `sdk/x1-sdk.js` |
| Native build configuration | `build.zig` |
| npm artifact assembly | `sdk/scripts/package-libx1.mjs` |
...
```text
JavaScript createX1Agent()
        |
        v
sdk/node.js selects native backend
        |
        v
createCore(options) in libx1.node
        |
        v
...
newline-delimited ACP JSON-RPC
        |
        v
shared createX1Agent() logic in sdk/x1-sdk.js
```
...
| Export | Purpose |
| --- | --- |
| `libx1ApiVersion` | Checks compatibility with the JavaScript loader. Currently `2`. Low-level `createCore` backends must declare this exact version. |
...
This ABI is internal. Consumers should use `createX1Agent()` from `sdk/node.js`; exposing the primitive functions keeps the native boundary small and testable.
...
`RuntimeHandle.runtime` becomes null during destruction. Later operations fail with `LIBX1_NATIVE_CLOSED`, and repeated destruction is harmless.
...
The native core is intentionally more restricted than the native `x1` CLI. Its ACP server configuration sets:
...
As a result, the model receives no native tool advertisement, cannot launch commands, cannot read workspace files through x1 tools, cannot start ACP-provided MCP servers, and cannot access the native secret store. `home` and `workspaceRoot` still provide identity and session context to shared ACP code, but they do not grant a tool capability by themselves.
...
## LayerX1 Responses endpoint policy

The Responses URL is validated independently in JavaScript and Zig. This duplication is intentional defense in depth because callers can load and call `libx1.node` directly, bypassing `sdk/node.js`.

Accepted endpoints are:

- the exact canonical production LayerX1 Responses URL; or
...
Input overflow fails synchronously with `LIBX1_NATIVE_BACKPRESSURE`. Output overflow causes the ACP runtime to exit with a failure status rather than allowing unbounded native memory growth. Limits must remain checked with overflow-safe subtraction before append operations.
...
A `.node` addon is executable native code loaded into the Node process. N-API provides ABI stability, not sandboxing. A compromised or substituted addon has the full authority of the host process regardless of the x1 capability restrictions described above.
...
The JavaScript loader searches for `libx1.node` first for local development, then `libx1.<platform>-<arch>.node` for packaged builds. It validates `libx1ApiVersion` and the expected export shape before use. `backend: "native"` fails closed if a compatible addon is unavailable. `backend: "auto"` may fall back to WebAssembly when JSPI is available.
...
`addNapiArtifact()` builds `src/napi_core_main.zig` as a stripped dynamic library, links libc, includes `node_api.h`, allows unresolved shared-library symbols for Node to resolve, and installs the artifact as `zig-out/lib/libx1.node`.
...
| Code | Meaning |
| --- | --- |
| `LIBX1_INVALID_ARGUMENT` | Missing, mistyped, oversized, invalid, or forged input. |
| `LIBX1_NATIVE_LIMIT` | The process-wide runtime limit was reached. |
| `LIBX1_NATIVE_BACKPRESSURE` | The bounded input queue cannot accept more bytes. |
| `LIBX1_NATIVE_CLOSED` | An operation targeted a closed runtime. |
| `LIBX1_NATIVE_OOM` | Native allocation failed. |
| `LIBX1_NATIVE_THREAD` | Runtime thread creation failed. |
| `LIBX1_NATIVE_IO` | Native queue or Buffer transfer failed. |
| `LIBX1_NAPI` | A Node-API operation failed unexpectedly. |

The JavaScript loader adds `LIBX1_NATIVE_UNAVAILABLE` for forced-native selection failures and `LIBX1_JSPI_REQUIRED` when neither native execution nor JSPI-backed WebAssembly is available.
