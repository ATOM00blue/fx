...
const runtime = await createX1Terminal({
  ...
  env: { X1_API_KEY: "term-headless-key" },
  fetch: mockFetch,
...
while (!readGrid().includes("layerx1.com")) {
  await flushTerminal();
  if (performance.now() >= startupDeadline) throw new Error(`timed out waiting for x1-term startup:\n${readGrid()}`);
...
while (!readGrid().includes("streamed response")) {
  await flushTerminal();
  if (performance.now() >= streamDeadline) throw new Error(`timed out waiting for rendered x1-term stream; requestedModel=${requestedModel}; events=${JSON.stringify(events)}:\n${readGrid()}`);
  await new Promise((resolve) => setTimeout(resolve, 10));
}
await flushTerminal();
const grid = readGrid();
...
if (!grid.includes("layerx1.com")) throw new Error(`shared X1 welcome frame was not visible in xterm grid:\n${grid}`);
...
console.log("headless xterm smoke passed: shared x1 frame used the 96x30 host cell grid");
