...
This file is maintainer guidance, not consumer documentation. Keep setup and the supported public surface in `sdk/README.md`. Keep exact API shapes in `sdk/x1-sdk.js` and its tests. Add details here only when they help an agent choose the right owner, preserve a non-obvious invariant, or run the right proof. The native Node-API architecture and security model are documented in `sdk/NAPI.md`.
...
| --- | --- |
| Shared loader, host imports, adapters, and public JavaScript exports | `sdk/x1-sdk.js` |
| Headless ACP composition for `x1-core.wasm` | `src/wasm_core_main.zig` |
| Interactive terminal entry point for `x1-term.wasm` | `src/wasm_term_main.zig` and `runWasmTerminal` in `src/main.zig` |
| Native and WebAssembly capability policy | `src/core/hosts/runtime_profile.zig` |
| Host-backed terminal session persistence | `src/core/app/app_session_runtime.zig` and `sdk/x1-sdk.js` |
...
| Browser device login, OAuth session persistence, and URL opening | `src/core/auth/js_host_auth.zig`, `src/core/auth/layerx1_session.zig`, and `src/core/hosts/js_host_url_opener.zig` |
...
- Keep `sdk/x1-sdk.js` a dependency-free ECMAScript module. A bundler, framework, or runtime package must not become necessary to load the SDK.
- Keep the core and terminal surfaces distinct. `x1-core.wasm` starts the headless ACP server; `x1-term.wasm` starts the interactive terminal. Shared loader changes must be validated against both.
...
- Treat JavaScript host stores as durable contracts. Session and OAuth snapshots are opaque bytes with optimistic revisions. Preserve `X1_SESSION_REVISION_CONFLICT` and `X1_OAUTH_SESSION_REVISION_CONFLICT`. Persist configuration only after x1 accepts it, and do not collapse prompt-history outcomes into generic success.
...
- `window.__x1CoreTest` and `document.body.dataset.state` are test interfaces for the core debugger. If either changes intentionally, update the browser test in the same change.
...
| `createX1Agent()`, ACP translation, core session persistence, streaming, or cancellation | Core build, core Node tests, and the browser test when browser behavior is involved |
...
For a live Gateway transport change, `X1_API_KEY` must already be present in the environment before running the opt-in smoke test:
