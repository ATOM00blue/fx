...
def profile_use_argv(
    ...
    flags = USE_FLAGS if paths.selector == "x1" else BENCHMARK_USE_FLAGS
    return (
        str(toolchain.opt),
        *flags,
...
def candidate_object_argv(
    ...
    outliner_flags = X1_MACHINE_OUTLINER_FLAGS if paths.selector == "x1" else ()
    return (
        str(toolchain.llc),
        "-filetype=obj",
        "-O=2",
