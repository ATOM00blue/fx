...
pub const FrameSurface = struct {
    ...
    /// terminal lines, and x1 keeps autowrap disabled session-wide.
    ...
    /// Removes x1-owned color, emphasis, and hyperlink presentation while
    /// preserving glyphs, geometry, ownership, and shell-owned cells.
    pub fn neutralizex1OwnedPresentation(self: *FrameSurface) void {
        for (self.cells) |*cell| {
    ...
    pub fn validate(self: FrameSurface) FrameSurfaceError!void {
        ...
        while (row <= self.rows) : (row += 1) {
            ...
            while (col <= self.cols) : (col += 1) {
                ...
                if (!self.ownerLegalAt(row, cell.owner)) return error.Unownedx1Cell;
...
test "frame surface presentation neutralization preserves shell cells and x1 geometry" {
    var shadow = try shadowGrid(std.testing.allocator, 8, 6);
    defer shadow.deinit();
    ...
    _ = try surface.writeAnsiBand(
        2,
        1,
        "\x1b[1;32mF\x1b]8;;https://x1.example\x1b\\X\x1b]8;;\x1b\\\x1b[0m",
        .transcript,
        .same_owner,
    );
    const preserved_before = surface.cellAt(1, 1).?;
    const x1_before = surface.cellAt(2, 1).?;
    try std.testing.expect(!x1_before.style.eql(.{}));

    surface.neutralizex1OwnedPresentation();

    const preserved_after = surface.cellAt(1, 1).?;
    const x1_after = surface.cellAt(2, 1).?;
    try std.testing.expect(std.meta.eql(preserved_before, preserved_after));
    try std.testing.expectEqual(x1_before.codepoint, x1_after.codepoint);
    try std.testing.expectEqual(x1_before.width, x1_after.width);
    try std.testing.expectEqual(x1_before.combining_suffix_id, x1_after.combining_suffix_id);
    try std.testing.expectEqual(x1_before.owner, x1_after.owner);
    try std.testing.expect(x1_after.style.eql(.{}));
}
...
test "full-terminal scroll invalidation does not allow painting preserved shell rows" {
    ...
    try std.testing.expectError(error.WriteOutsideBand, surface.writeAnsiBand(1, 1, "x1", .transcript, .same_owner));
    try std.testing.expectEqual(paint_plan.CellOwner.preserved_shell, surface.cellAt(1, 1).?.owner);
}
