...
fn composeSelectionFailureRow(
    ...
    const message = switch (failure) {
        .open_elsewhere => "This session is open in another x1. Close it there, then press Enter to retry.",
...
test "resume menu renders each session on one line with a right metadata cluster" {
    ...
    const summaries = [_]session_store.SessionSummary{.{
        .id = @constCast("one"),
        .workspace_root = @constCast("/Users/example/Developer/x1/worktrees/resume-catalog"),
...
test "resume menu explains retryable selected session contention" {
    ...
    const cases = [_]struct {
        ...
        .{
            ...
            .message = "This session is open in another x1. Close it there, then press Enter to retry.",
        },
        .{
...
test "resume menu keeps long titles on one narrow row without a workspace spill row" {
    ...
    const summaries = [_]session_store.SessionSummary{.{
        .id = @constCast("one"),
        .workspace_root = @constCast("/Users/example/Developer/x1/worktrees/a-very-long-session-catalog-worktree"),
