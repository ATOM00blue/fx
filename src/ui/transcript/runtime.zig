...
fn lifecycleMarker(kind: types.ToolOutcomeKind) LifecycleMarker {
    return switch (kind) {
        .completed => .{ .style = ui_render.diff_added_marker_style, .glyph = "●" },
...
fn lifecycleTerminalLine(
    ...
    const follow_up = if (cancelled) " · What can x1 do differently?" else "";
...
fn checkToolDetailResultReplacementAllocationFailures(alloc: Allocator) !void {
    ...
    var previous_artifact: ?[]u8 = try alloc.dupe(u8, "x1-command-previous.log");
...
test "historical command detail keeps artifact handles after command block attaches" {
    ...
    try std.testing.expectEqualStrings("x1-command-replayed.log", fallback_detail.command_artifact_handle.?);
    ...
    try std.testing.expectEqualStrings("x1-command-replayed.log", command_detail.command_artifact_handle.?);
...
test "historical nonzero command keeps replay ownership outside compact sidebands" {
    ...
    const replay_handle = "x1-command-replay-historical.bin";
...
test "historical command detail ignores artifact-shaped command output" {
    ...
    const raw_result = "exit_code=0\n<stdout>\noutput_file=/tmp/x1-command-other.log\n</stdout>\n";
    var metrics: Metrics = .{};
    const entry_id = try runtime.writeCompletedToolStatusReturningEntryId(
        alloc,
        &metrics,
...
test "command terminal detail owns the consolidated command output entry" {
    ...
    _ = try runtime.applyToolLifecycle(alloc, .{ .terminal = .{
        ...
        .command_artifact_handle = "x1-command-live.log",
    } });
    ...
    try std.testing.expectEqualStrings("x1-command-live.log", detail.command_artifact_handle.?);
}
...
pub const TranscriptRuntime = struct {
    stdout_file: std.Io.File = if (builtin.os.tag == .windows)
        undefined
    else
        std.Io.File.stdout(),
    ...
    cursor_row: u16 = 1,
    cursor_col: u16 = 1,
    /// Top terminal row owned by x1. Rows above this contain pre-x1 scrollback
    /// and must not be touched by x1 repaint logic.
    ...
    /// First row x1 is allowed to clear or repaint. This is set after
    /// launch-time reservation has created x1-owned rows. Rows above it
    /// belong to pre-x1 terminal content and must not be touched by
    ...
    /// Minimum body rows x1 should keep available for the visible
    ...
    /// next replay. Real terminals can reflow old x1 cells just above
    ...
    /// x1's transcript band. Before that point transcript writes are
    /// model updates only: emitting scroll newlines would move pre-x1
    ...
    /// Bootstrap enables it unconditionally; X1_TRACE dumps it at synchronized
    /// update boundaries.
...
test "resume view snapshot omits a visible leading welcome entry" {
    ...
    try runtime.writeTranscriptClassified(
        alloc,
        &metrics,
        "x1 · Run /help for commands\n\n",
        true,
        .welcome,
    );
    try runtime.enableShadowVt(alloc);
    try runtime.shadow_vt.?.feed(
        "\x1b[1;1Hx1 · Run /help for commands" ++
