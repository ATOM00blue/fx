...
const supports_headless_interrupt = switch (std_builtin.os.tag) {
    ...
    // Windows delivers Ctrl+C through the shared console-control handler
    // instead of a SIGINT disposition.
    .windows => true,
    else => true,
};
...
const headless_interrupt = if (std_builtin.os.tag == .windows) struct {
    ...
    /// Invoked from the OS console-control handler thread when a
    /// CTRL_C_EVENT arrives while a headless scope owns the console.
    fn requestInterrupt() callconv(.c) void {
    ...
    const Scope = struct {
        ...
        fn install(enabled: bool) HeadlessInterruptInstallError!Scope {
            ...
            // Shares the console-control registration with the
            // interactive abnormal-exit layer when both are installed.
            app_lifecycle.windows_console_control.setHeadlessInterruptRequest(&requestInterrupt);
        ...
        fn restore(self: *Scope, redeliver: bool) void {
            ...
            app_lifecycle.windows_console_control.setHeadlessInterruptRequest(null);
            coordinator_active = false;
            self.installed = false;
            if (redeliver and cancel_requested.load(.seq_cst)) {
                // POSIX re-raises the consumed signal so the parent sees
                // the default disposition; Windows exits with the code a
...
/// Resume selector parsed from x1 ask --resume.
...
const AskContext = struct {
    ...
    fn configureNotifications(
        ...
        if (attention_required) {
            try self.lifecycle_runtime.registerAttentionRequired(.{
                .name = "x1.sound.attention_required",
                .ctx = self,
    ...
    fn initializeSessionStores(self: *AskContext) !void {
        var store = session_store.Store.init(self.alloc, self.workspace_root) catch |err| {
            ...
            try self.writeStderr("x1 ask: warning: session persistence unavailable; error=");
    ...
    fn toolContext(self: *AskContext) tool_runtime.Context {
        ...
        var tc: tool_runtime.Context = .{
            ...
            .web_search_backend = if (provider_capabilities.provider_search) self.web_search_runtime.dispatchBackend() else null,
...
fn writeAskUsage(deps: RunDeps, usage: []const u8) !void {
    try deps.write_stderr(deps.stderr_ctx, "usage: x1 ");
...
fn runWithDeps(alloc: Allocator, args: []const [:0]const u8, cfg: Config, deps: RunDeps) !u8 {
    ...
    var options = parseOptionsWithStdin(alloc, args, deps.stdin_source) catch |err| switch (err) {
        error.MissingPrompt => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "x1 ask: missing prompt\n");
        ...
        error.NoSaveResumeConflict => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "x1 ask: --no-save cannot be used with --resume or --resume-id\n");
            try writeAskUsage(deps, cfg.command_usage);
            return 1;
        },
        error.PromptResourceLimitExceeded => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "x1 ask: prompt exceeds the local input safety limit\n");
            return 1;
        },
        error.PromptInputReadFailed => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "x1 ask: failed to read prompt from stdin\n");
            return 1;
        },
        ...
        error.InvalidPromptText => {
            ...
            try deps.write_stderr(deps.stderr_ctx, "x1 ask: prompt must be valid UTF-8 and contain no NUL bytes\n");
            return 1;
        },
        else => return err,
    };
    ...
    const result = runPromptInternal(alloc, options.prompt, options.permission_override, effective_cfg, .{
        ...
        if (err == error.OneOffSessionNotResumable and !options.json_output) {
            try deps.write_stderr(
                deps.stderr_ctx,
                "x1 ask: one-off child sessions cannot accept additional prompts; create a persistent child to continue the conversation\n",
            );
            return 1;
        }
        if (!options.json_output) {
            const notice = askErrorNotice(err) orelse return err;
            try deps.write_stderr(deps.stderr_ctx, "x1 ask: ");
    ...
    for (options.image_paths.items) |image_path| {
        const attachment = image_attachments.loadUserImageAttachment(
            ...
            if (options.json_output) {
                ...
                try message.writer.print("x1 ask: failed to attach image \"{s}\": {s}\n", .{ image_path, reason });
...
fn missingCredentialResult(
    ...
    _ = provider;
    const message = credentials.missing_layerx1_credential_message;
    try options.deps.write_stderr(options.deps.stderr_ctx, "x1 ask: ");
...
fn finishCliPermissionOutcome(
    ...
    switch (outcome.requirement orelse .approval_required) {
        .configured_rule => try writeBlockedActionGuidance(
            ctx,
            "x1 ask: permission required by configured rule",
            label,
            "noninteractive_permission_prompt_unavailable",
            "x1 ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        ),
        .approval_required => try writeBlockedActionGuidance(
            ctx,
            "x1 ask: permission required for tool execution in noninteractive mode",
            label,
            "noninteractive_permission_prompt_unavailable",
            if (permission_mode == .auto)
                "x1 ask: human approval is required for this action; use the interactive shell to approve it, or add a narrow matching permission rule\n"
            else
                "x1 ask: rerun with --auto to review this exact action automatically, or use the interactive shell to approve it\n",
        ),
    }
...
fn writeBlockedActionGuidance(
    ...
    try ctx.writeStderr("x1 ask: blocked action: ");
...
fn emitAskNotificationBell(raw: *anyopaque) void {
    ...
    ctx.writeStderr("\x07") catch |err| {
        debug_trace.logf(
            "notifications",
            "x1 ask terminal bell write failed err={s}",
            .{@errorName(err)},
        );
    };
}
    ...
    switch (attempt) {
        ...
        .failure => |failure| {
            ...
            try message.writer.print(
                "x1 ask: failed to save YOLO acknowledgment: {s}\n",
...
fn promptRealPermissionApproval(
    ...
    try write_stderr(stderr_ctx, "x1 wants to run:\n  ");
...
fn testMissingKeyStartup(alloc: Allocator, _: oauth_transport.Provider, _: host.SecretStore, default_model: []const u8, default_agent_step_limit: usize) !app_lifecycle.StartupState {
    ...
    state.workspace_root = try alloc.dupe(u8, "/tmp/x1-test");
...
fn testProcessQueuedPrompt(deps: *const agent_runtime.AgentRuntimeDeps, semantic_presentation: ?agent_runtime.SemanticPresentationSink, lifecycle: agent_runtime.LifecycleContext, _: agent_runtime.Config, _: worker_runtime.QueuedPrompt) !void {
    ...
    try std.testing.expectEqualStrings("/tmp/x1-test", lifecycle.scope.workspace_root);
...
test "x1 ask deps validate malformed registered calls" {
    const alloc = std.testing.allocator;
...
test "x1 ask X1 route disables Gateway-backed auxiliary providers" {
    const alloc = std.testing.allocator;
    var stdout_capture = TestCapture{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture = TestCapture{};
    ...
    defer ctx.deinit();
    ctx.api_key = "x1-secret";
    ctx.credential_source = .layerx1_subscription;
    ctx.provider = .layerx1;
    ctx.model = "gpt-5.4";
...
test "x1 ask finalization fails every failed turn" {
...
test "headless yolo warning precedes startup configuration diagnostics" {
    ...
    try std.testing.expect(std.mem.startsWith(
        ...
        stderr_capture.bytes.items,
        permissions.yolo_warning_text ++ "\n" ++
            "x1 ask: config user: malformed_settings\n",
    ));
}
...
test "runWithDeps reports a missing image before startup after cleaning prior attachments" {
    ...
    const missing_path_z = try alloc.dupeZ(u8, "/tmp/x1-ask-missing-image.png");
...
test "stdin read failure has distinct text and JSON output contracts" {
    ...
    try std.testing.expectEqualStrings(
        "x1 ask: failed to read prompt from stdin\n",
        stderr_capture.bytes.items,
    );
...
test "x1 ask default user commands require configured authority or review" {
    const alloc = std.testing.allocator;
...
test "x1 ask automatic review observes worker cancellation" {
    ...
    defer stderr_capture.deinit(alloc);
    var cfg = testConfig();
    cfg.provider_set.layerx1.permission_reviewer = .{ .review_fn = Provider.review };
    var ctx = AskContext.init(
        alloc,
        cfg,
        testPromptRunDeps(&stdout_capture, &stderr_capture, testPresentKeyStartup),
...
test "x1 ask auto mode applies automatic clear and caution without a prompt" {
...
test "x1 ask terminal permission prompt approves and denies run_command" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "x1 ask permission attention fires once after a prompt is published" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "x1 ask bell writes only to TTY stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "x1 ask registers notification handlers only when configured" {
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(std.testing.allocator);
    var stderr_capture: TestCapture = .{};
    defer stderr_capture.deinit(std.testing.allocator);
...
test "x1 ask captured and quiet permission paths bypass terminal prompt" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "x1 ask permission prompt policy requires explicit captured-mode opt in and TTY stdin" {
    const Case = struct {
        output_mode: OutputMode,
        prompt_permissions: bool,
        stdin_is_tty: bool,
...
test "x1 ask captured permission prompt opt in uses the existing prompter" {
    const alloc = std.testing.allocator;
    var arena_state = std.heap.ArenaAllocator.init(alloc);
    defer arena_state.deinit();
    const arena = arena_state.allocator();
...
test "x1 ask terminal permission prompt propagates prompt hook errors" {
...
test "x1 ask prepared file mutation callback preserves terminal permission prompt" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
...
test "x1 ask auto mode uses automatic allow for external prepared file mutation" {
    const FakeClassifier = struct {
        calls: usize = 0,
        root_text: []const u8 = "",
...
test "x1 ask preserves CLI headless blocker diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    ...
    const expected_configured_stderr = try std.fmt.allocPrint(
        arena,
        "x1 ask: permission required by configured rule\n" ++
            "x1 ask: blocked action: {s}\n" ++
            "x1 ask: reason=noninteractive_permission_prompt_unavailable\n" ++
            "x1 ask: rerun in the interactive shell to approve this action, or add a narrow matching permission rule before retrying\n",
        .{configured_label},
    );
    try std.testing.expectEqualStrings("", stdout_capture.bytes.items);
...
test "x1 ask ordinary multi-target admission preserves deny precedence without diagnostics" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "x1 ask renders one-off resume denial in text and JSON modes" {
    const alloc = std.testing.allocator;
    const cases = [_]struct {
...
test "saved ask classifies unsafe store failure by request mode" {
    ...
    try tmp.dir.createDirPath(io_mod.getIo(), "home");
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
    var blocker = try tmp.dir.createFile(io_mod.getIo(), "home/.x1", .{});
    ...
    try std.testing.expectEqualStrings(
        "x1 ask: warning: session persistence unavailable; error=SessionPathUnsafe; continuing without saving\n",
        stderr_capture.bytes.items,
    );
...
test "x1 ask JSON captures HTTP 413 prompt-too-long blocker" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "x1 ask: HTTP 413: provider payload rejected") != null);
...
test "x1 ask formats restricted-provider HTTP errors without raw JSON" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expect(std.mem.find(u8, stderr_capture.bytes.items, "x1 ask: API access denied · HTTP 403 · Provider: wafer") != null);
...
test "x1 ask text and JSON share the selected auth failure facts" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings(
        "x1 ask: X1 subscription authentication failed · HTTP 401\n",
        stderr_capture.bytes.items,
    );
    var parsed = try std.json.parseFromSlice(std.json.Value, alloc, stdout_capture.bytes.items, .{});
    defer parsed.deinit();
    ...
    try std.testing.expectEqualStrings("X1 subscription", auth_failure.get("source").?.string);
...
test "saved API key 401 discards the fresh pristine session" {
    ...
    var store = try session_store.Store.initFromHome(alloc, home, "/tmp/x1-test");
    defer store.deinit(alloc);
    var sessions = try store.list(alloc);
    defer {
        for (sessions.items) |*summary| summary.deinit(alloc);
    ...
    try std.testing.expectError(
        ...
        store.resumeTargetForWrite(alloc, .last, "/tmp/x1-test", .{}),
    );
}
...
test "x1 ask JSON records permission-denied tool calls as error status" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(io_mod.getIo(), "workspace");
...
test "x1 ask JSON permission-denied capture is best effort under allocation failure" {
    var failing = std.testing.FailingAllocator.init(
        std.testing.allocator,
        .{ .fail_index = 0 },
    );
...
test "x1 ask JSON captures parallel tool results without corrupting records" {
    const alloc = std.heap.c_allocator;
...
test "x1 ask JSON capture cleans up every allocation failure" {
...
test "x1 ask JSON records ask_user_question text for matching assertions" {
    const alloc = std.testing.allocator;
    const records = try alloc.alloc(ToolCallRecord, 1);
    records[0] = .{
...
test "x1 ask JSON clips ask_user_question text at a UTF-8 boundary" {
    const alloc = std.testing.allocator;
...
test "json run with missing API key prints diagnostic then final object" {
    ...
    try std.testing.expectEqual(@as(u8, 1), exit_code);
    try std.testing.expectEqualStrings("x1 ask: " ++ credentials.missing_layerx1_credential_message ++ "\n", stderr_capture.bytes.items);
    try std.testing.expectEqualStrings(
...
test "recovery continuation checks local checkpoint before credentials" {
    ...
    var store = try session_store.Store.initFromHome(alloc, home, "/tmp/x1-test");
    defer store.deinit(alloc);
    var state = try testAskDurableState(alloc, "/tmp/x1-test", session_id);
    defer state.deinit(alloc);
    var writable = try store.startWritableSession(alloc, state);
    writable.deinit(alloc);
...
test "x1 ask emits one discovery warning when catalog truncation and a skill read report it" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "x1 ask carries resolved auto mode and initial registry context into the queued prompt" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default x1 ask passes canonical image paths as initial context targets" {
    const alloc = std.testing.allocator;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    {
...
test "default x1 ask omits disabled registry context without gathering" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "default x1 ask preserves project context gathering error mappings" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "x1 ask JSON recovery keeps stdout structured and reports progress on stderr" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "x1 ask JSON reports the consumed attempt after retry admission failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "x1 ask JSON preserves partial output on prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
...
test "x1 ask raw output still propagates prompt failure" {
    const alloc = std.testing.allocator;
    var stdout_capture: TestCapture = .{};
    defer stdout_capture.deinit(alloc);
    var stderr_capture: TestCapture = .{};
