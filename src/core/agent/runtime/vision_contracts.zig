...
pub const provider_response_format_name = "x1_vision_evidence";
...
test "Vision result serialization emits the five x1-owned diagnostics" {
    const Case = struct {
        failure: VisionFailure,
        expected: []const u8,
    };
...
test "Vision provider cannot mint x1-owned local diagnostic codes" {
    inline for (.{
        "image_unavailable",
        "provider_response_invalid",
        "output_limit_exceeded",
