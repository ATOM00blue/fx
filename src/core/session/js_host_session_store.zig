...
extern "x1" fn x1_session_commit(
    id_ptr: [*]const u8,
    id_len: usize,
    bytes_ptr: [*]const u8,
    bytes_len: usize,
...
extern "x1" fn x1_session_list(
    response_ptr: [*]u8,
    response_cap: usize,
) i32;

extern "x1" fn x1_session_remove(
    id_ptr: [*]const u8,
    id_len: usize,
) i32;
...
pub fn load(alloc: Allocator, id: []const u8) !?Loaded {
    ...
    while (true) {
        ...
        const encoded_len = x1_session_load(
            id.ptr,
            id.len,
            encoded.ptr,
            encoded.len,
...
pub fn commit(
    ...
    const status = x1_session_commit(
        state.id.ptr,
        state.id.len,
        encoded.written().ptr,
        encoded.written().len,
...
pub fn list(alloc: Allocator) ![]Metadata {
    ...
    while (true) {
        ...
        const response_len = x1_session_list(response.ptr, response.len);
        if (response_len == -2) {
...
pub fn remove(id: []const u8) !void {
    const status = x1_session_remove(id.ptr, id.len);
