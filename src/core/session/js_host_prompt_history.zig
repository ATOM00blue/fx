...
extern "x1" fn x1_prompt_history_available() i32;

extern "x1" fn x1_prompt_history_load(
...
extern "x1" fn x1_prompt_history_append(
    timestamp_ms: i64,
    workspace_ptr: [*]const u8,
    workspace_len: usize,
    text_ptr: [*]const u8,
...
extern "x1" fn x1_prompt_history_clear(
    workspace_ptr: [*]const u8,
    workspace_len: usize,
) i32;

pub fn available() bool {
    return x1_prompt_history_available() == 1;
}
...
fn loadRecent(
    ...
    while (capacity <= max_response_bytes) : (capacity *= 2) {
        ...
        const result = x1_prompt_history_load(
            workspace_root.ptr,
            workspace_root.len,
            limit,
            buffer.ptr,
...
fn append(
    ...
    return switch (x1_prompt_history_append(
        timestamp_ms,
        workspace_root.ptr,
        workspace_root.len,
        text.ptr,
...
fn clearWorkspace(
    ...
    if (x1_prompt_history_clear(workspace_root.ptr, workspace_root.len) != 0) {
        return error.PromptHistoryHostFailure;
    }
}
