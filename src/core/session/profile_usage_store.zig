...
fn openExistingUsageFile(
    ...
    if (comptime builtin.os.tag == .windows) {
        return io_mod.openExistingRegularFile(dir, usage_file, mode) catch error.UsageReadFailed;
    } else {
...
test "profile usage store leaves an incomplete tail intact when repair exceeds record capacity" {
    ...
    var profile = try tmp.dir.openDir(io_mod.getIo(), ".x1", .{ .iterate = true });
...
test "profile usage store refuses a symlinked ledger leaf" {
    ...
    try tmp.dir.deleteFile(io_mod.getIo(), ".x1/usage.jsonl");
