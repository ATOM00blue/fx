...
/// Returns an owned path another process can use to launch x1. Linux prefers
...
fn testProductExe() ?[]const u8 {
    ...
    const path_z = std.c.getenv("X1_TEST_PRODUCT_EXE") orelse return null;
...
test "on-disk probe rejects a replaced binary so the peer path falls back" {
    ...
    const victim = try std.fs.path.join(alloc, &.{ dir_path, "x1-probe" });
    ...
    try std.testing.expect(!onDiskPathIsExecutable("x1"));
}
