...
fn loadOptionsFromEnv(alloc: Allocator, workspace_root: []const u8) !Options {
    const trace_log = loadOptionalEnv("X1_TRACE_LOG");
    const trace_flag = loadOptionalEnv("X1_TRACE");
    const trace_stderr = loadOptionalEnv("X1_TRACE_STDERR");
    const trace_scopes = loadOptionalEnv("X1_TRACE_SCOPES");
...
fn fallbackLogPathForMillis(alloc: Allocator, millis: i64) ![]u8 {
    return std.fmt.allocPrint(alloc, "/tmp/x1-trace-{d}.log", .{millis});
}
...
test "home default log path uses x1 logs directory" {
    const alloc = std.testing.allocator;
    const path = try defaultLogPathForHome(alloc, "/tmp/fake-home");
    defer alloc.free(path);
    try std.testing.expectEqualStrings("/tmp/fake-home/.x1/logs/trace.log", path);
}
...
test "fallback default log path uses tmp trace path" {
    ...
    defer alloc.free(path);
    try std.testing.expectEqualStrings("/tmp/x1-trace-12345.log", path);
}
...
test "keylessJsonPreview reports shape without keys or values" {
    const alloc = std.testing.allocator;
    const preview_text = try keylessJsonPreview(alloc, "{\"X1_DYNAMIC_PATH\":\"secret.txt\",\"X1_DYNAMIC_CONTENT\":\"very secret\",\"nested\":{\"X1_DYNAMIC_TOKEN\":\"abc\"}}");
    defer alloc.free(preview_text);

    try std.testing.expect(std.mem.find(u8, preview_text, "<object_fields=3 values=[") != null);
    try std.testing.expect(std.mem.find(u8, preview_text, "X1_DYNAMIC_PATH") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "X1_DYNAMIC_CONTENT") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "X1_DYNAMIC_TOKEN") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "secret.txt") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "very secret") == null);
    try std.testing.expect(std.mem.find(u8, preview_text, "abc") == null);
}
...
test "resolveLogPath resolves absolute and relative paths" {
    const alloc = std.testing.allocator;
    const absolute = try resolveLogPath(alloc, "/tmp/workspace", " \t/tmp/x1-absolute-trace.log\n");
    defer alloc.free(absolute);
    try std.testing.expectEqualStrings("/tmp/x1-absolute-trace.log", absolute);
...
test "configureFromEnv leaves tracing disabled without env" {
    resetForTest();
    defer resetForTest();
    try std.testing.expect(io_mod.getenv("X1_TRACE_LOG") == null);
    try std.testing.expect(io_mod.getenv("X1_TRACE") == null);
    try std.testing.expect(io_mod.getenv("X1_TRACE_STDERR") == null);
