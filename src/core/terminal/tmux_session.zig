...
pub const launcher_mode = "--x1-internal-terminal-tmux-launcher";
pub const capture_mode = "--x1-internal-terminal-tmux-capture";

const namespace_option = "@x1_terminal_namespace";
...
const PeerDeadline = struct {
    ...
    fn init(default_ms: i64) PeerDeadline {
        const duration_ms = if (io_mod.getenv(
            "X1_TERMINAL_TEST_TMUX_DEADLINE_MS",
        )) |text|
            @min(
...
fn cleanupOwnedNamespaceWithEvidence(
    ...
    if (session_present) {
        ...
        if (io_mod.getenv("X1_TERMINAL_TEST_FAIL_TMUX_CLOSE_CLEANUP") != null) {
...
pub fn runCapture(raw_args: []const [*:0]const u8) !void {
    ...
    const test_failure = io_mod.getenv("X1_TERMINAL_TEST_TMUX_CAPTURE_FAILURE");
...
fn assignForegroundProcessGroup(fd: c_int, pgrp: std.posix.pid_t) bool {
    if (io_mod.getenv("X1_TERMINAL_TEST_TMUX_TCSETPGRP_FAILURE") != null) {
        return false;
    }
...
fn requestChildTermination(child_pid: std.posix.pid_t) void {
    const group_kill_succeeded =
        io_mod.getenv("X1_TERMINAL_TEST_TMUX_GROUP_KILL_FAILURE") == null and
...
test "tmux paths keep artifacts durable and place only the server socket in transport" {
    const alloc = std.testing.allocator;
    const durable_root = "/profiles/example/.x1/terminal-host";
    const transport_root = "/tmp/x1-terminal-501-profile";
    const identity = "0123456789abcdef0123456789abcdef";
    var paths = try Paths.init(
        alloc,
        durable_root,
    ...
    try std.testing.expectEqualStrings(
        "/tmp/x1-terminal-501-profile/tmux.sock",
        paths.socket,
    );
    for ([_][]const u8{
        paths.config,
