...
pub const internal_mode = "--x1-internal-terminal-host";
...
const transport_hash_context = "x1.terminal.transport.v1\x00";
...
pub const Config = struct {
    ...
    pub fn fromEnvironment(
        ...
        if (io_mod.getenv("X1_TERMINAL_HOST_PROTOCOL_MIN")) |value| {
            config.hello.range.minimum = try std.fmt.parseInt(u16, value, 10);
        }
        if (io_mod.getenv("X1_TERMINAL_HOST_PROTOCOL_CURRENT")) |value| {
            config.hello.range.current = try std.fmt.parseInt(u16, value, 10);
        }
        if (io_mod.getenv("X1_TERMINAL_HOST_PROTOCOL_CAPABILITIES")) |value| {
            config.hello.capabilities = try std.fmt.parseInt(u64, value, 10);
        }
        if (io_mod.getenv("X1_TERMINAL_HOST_IDLE_MS")) |value| {
...
pub const Paths = struct {
    ...
    pub fn open(alloc: Allocator, home: []const u8) !Paths {
        ...
        const uid: std.c.uid_t = if (comptime builtin.os.tag == .windows) 0 else std.c.getuid();
        ...
        var x1_dir = try io_mod.openOrCreateVerifiedPrivateDir(
            &home_dir,
...
fn runSupported(alloc: Allocator, config: Config) !void {
    ...
    maybeDelayForTest("X1_TERMINAL_TEST_IDLE_EXIT_DELAY_MS");
...
fn testRequestFailureRequested(
    ...
    const requested_point = io_mod.getenv("X1_TERMINAL_TEST_HOST_FAILURE_POINT") orelse
        return false;
    if (!std.mem.eql(u8, requested_point, point)) return false;
    const requested_correlation = io_mod.getenv(
        "X1_TERMINAL_TEST_HOST_FAILURE_CORRELATION",
    ) orelse return false;
...
fn testAcceptFailureRequested() bool {
    const path = io_mod.getenv("X1_TERMINAL_TEST_ACCEPT_FAILURE_PATH") orelse
        return false;
...
fn noteTestOrderedAdmission(correlation_id: contracts.CorrelationId) !void {
    const prefix = io_mod.getenv("X1_TERMINAL_TEST_ORDER_BARRIER") orelse return;
...
fn awaitTestOrderedBoundary(
    ...
    const first_target = testCorrelationFromEnvironment(
        "X1_TERMINAL_TEST_ORDER_HOLD_CORRELATION",
    );
    const second_target = testCorrelationFromEnvironment(
        "X1_TERMINAL_TEST_ORDER_HOLD_CORRELATION_2",
    );
...
test "hidden host mode is exact and remains private" {
    const exact = [_][*:0]const u8{ "x1", internal_mode };
    try std.testing.expect(isInternalModeRaw(&exact));
    const public_like = [_][*:0]const u8{ "x1", "terminal-host" };
    try std.testing.expect(!isInternalModeRaw(&public_like));
    const extra = [_][*:0]const u8{ "x1", internal_mode, "extra" };
