...
pub const service_name = "X1_AI_GATEWAY_API_KEY";
const mcp_credentials_service_name = "X1_MCP_OAUTH_CREDENTIALS_V1";
/// Legacy Vercel OAuth session keychain item. Product auth uses
/// `~/.x1/layerx1-auth.json` and never migrates this item.
pub const oauth_session_service_name = "X1_OAUTH_SESSION_V1";
...
pub fn isDisabled() bool {
    const value = io_mod.getenv("X1_DISABLE_KEYCHAIN") orelse return false;
...
const test_service_name = "X1_TEST_AI_GATEWAY_API_KEY";
...
test "MCP Keychain storage round-trips values beyond the security prompt limit" {
    ...
    const alloc = std.testing.allocator;
    const test_mcp_service = "X1_TEST_MCP_OAUTH_CREDENTIALS_V1";
