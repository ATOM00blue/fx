...
pub fn renderTopLevelHelpWithStyle(alloc: Allocator, registry: TopLevelRegistry, columns: usize, version: []const u8, style: HelpStyle) ![]u8 {
    ...
    try writeStyled(&out.writer, style, .brand, "X1");
    ...
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "x1 [flags]", width, style, .syntax);
    try writeWrappedStyledLine(&out.writer, "  ", "  ", "x1 <command> [...flags] [...args]", width, style, .syntax);
...
test "rendered top-level help is a complete CLI navigation page" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, text, "X1 v9.8.7\nFast, native coding agent for the terminal."));
    try std.testing.expect(std.mem.find(u8, text, "X1 starts an interactive session by default.") != null);
    try std.testing.expect(std.mem.find(u8, text, "x1 <command> [...flags] [...args]") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "LayerX1 plan") != null);
    try std.testing.expect(std.mem.find(u8, text, "dollar balance") != null);
    try std.testing.expect(std.mem.find(u8, text, "month usage") != null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "X1_EXPERIMENTAL_WORKSPACE_ACCESS=1") == null);
    ...
    try std.testing.expect(std.mem.find(u8, text, "x1 ask \"Explain the changes in this repository\"") != null);
    try std.testing.expect(std.mem.find(u8, text, "x1 session resume last") != null);
    try std.testing.expect(std.mem.find(u8, text, "session resume [last|id]") != null);
    try std.testing.expect(std.mem.find(u8, text, "x1 status --json") != null);
    try std.testing.expect(std.mem.find(u8, text, "Run `/help` inside an interactive session for slash commands.") != null);
    try std.testing.expect(std.mem.find(u8, text, "Learn more about X1:") != null);
    try std.testing.expect(std.mem.find(u8, text, "https://layerx1.com") != null);
    try std.testing.expect(std.mem.find(u8, text, "\nReport a problem:     run `/feedback` inside X1\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "\n\n\nRun `x1 <command> --help`") == null);
...
test "terminal top-level help adds styling without changing visible content" {
    ...
    try std.testing.expect(std.mem.startsWith(u8, terminal, "\x1b[1mX1\x1b[0m"));
    ...
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[4mhttps://layerx1.com\x1b[0m") != null);
    try std.testing.expect(std.mem.find(u8, terminal, "\x1b[39mrun `/feedback` inside X1\x1b[0m") != null);
...
test "top-level help renders flags as compact aligned rows" {
    ...
    try std.testing.expect(std.mem.find(u8, wide, "Print the X1 version and exit\n\nExamples:") != null);
...
test "per-command help renders header usage options and details" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "x1 permissions\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  x1 permissions [--json]") != null);
...
test "per-command help preserves long resume usage outside top-level index" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  x1 session resume [last|<id>] [--record] | session resume --id <id> [--record] | --resume [last|<id>] [--record] | resume [last|<id>] [--record] | resume --id <id> [--record] | --resume-last | --continue | -c | -r | --resume-<id>") != null);
    try std.testing.expect(std.mem.find(u8, text, "Options:") != null);
...
test "ACP help documents accepted options" {
    ...
    try std.testing.expect(std.mem.find(u8, text, "x1 acp\n") != null);
    try std.testing.expect(std.mem.find(u8, text, "Usage:\n  x1 acp [--model <id>] [--log-file <path>]") != null);
...
test "interactive model command exposes plural alias" {
    const registry = testSlashRegistry();
...
test "slash completion descriptions follow completion matches" {
    ...
    try std.testing.expectEqualStrings("choose a live LayerX1 model and reasoning effort", nthSlashCompletionDescription(testSlashRegistry(), "/mo", 0).?);
    ...
    try std.testing.expectEqualStrings("open the x1 feedback form", nthSlashCompletionDescription(testSlashRegistry(), "/fee", 0).?);
