...
pub const user_agent = "x1/" ++ build_options.app_version;

pub fn networkFailureEvidence(
    err: anyerror,
    delivery: stream_provider.DeliveryCertainty.State,
) ?stream_provider.NetworkFailureEvidence {
    const cause: stream_provider.NetworkFailureCause = if (err == error.SystemResumed)
        .system_resumed
...
pub fn runBoundedHttpOperation(
    ...
    switch (event) {
        ...
        .deadline => |deadline_result| {
            ...
            Cleanup.drain(alloc, &select);
            return if (cancel_flag.load(.seq_cst)) error.Cancelled else error.Timeout;
        },
    }
}
...
pub fn spawnHttpCancelWatcher(
    ...
    return std.Thread.spawn(.{}, CancelWatcher.run, .{ done, cancel_flag, null, stream });
}
...
test "loopback override policy accepts only explicit local HTTP endpoints" {
    try std.testing.expect(isLoopbackHttpUrl("http://127.0.0.1:43123/v1/responses"));
    try std.testing.expect(isLoopbackHttpUrl("http://localhost:43123/v1/models"));
    try std.testing.expect(!isLoopbackHttpUrl("https://api.layerx1.com/v1/responses"));
    try std.testing.expect(!isLoopbackHttpUrl("http://127.0.0.1:43123@evil.test/v1/responses"));
}
