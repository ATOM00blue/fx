...
fn handleCommand(alloc: Allocator, rest: []const u8, command_request: CommandRequest) !CommandResult {
    ...
    if (std.mem.startsWith(u8, trimmed, "auth ")) {
        ...
        return switch (authentication) {
            .started => lineParts(
                alloc,
                &.{ "Waiting for MCP authentication for '", name, "'. You can continue using x1 while the browser flow completes." },
                false,
            ),
            .busy => lineParts(
                alloc,
...
test "saving MCP config replaces the file durably" {
    ...
    try writeTempFile(&tmp, "home/.x1/mcp.json", original);
    const path = try tmpDirPath(alloc, tmp.dir, "home/.x1/mcp.json");
    defer alloc.free(path);

    var x1_dir = try tmp.dir.openDir(io_mod.getIo(), "home/.x1", .{ .iterate = true });
    ...
    {
        var seed = try x1_dir.openFile(io_mod.getIo(), "mcp.json", .{ .mode = .read_write });
    ...
    var held = try x1_dir.openFile(io_mod.getIo(), "mcp.json", .{});
    ...
    const stat = try x1_dir.statFile(io_mod.getIo(), "mcp.json", .{ .follow_symlinks = false });
    try std.testing.expectEqual(@as(u32, 0o600), stat.permissions.toMode() & 0o777);

    var it = x1_dir.iterate();
    var entries: usize = 0;
...
test "MCP config diagnostic treats nonblocking profile states as clear" {
    ...
    try writeTempFile(&tmp, "home/.x1/mcp.json", "{\"mcp\":{}}");
    {
        const test_home = try TestHome.install(alloc, home_path);
        defer test_home.deinit();
...
test "MCP config diagnostic preserves the startup parser error" {
    ...
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try writeTempFile(&tmp, "home/.x1/mcp.json", "{invalid json");
    const home_path = try tmpDirPath(alloc, tmp.dir, "home");
    defer alloc.free(home_path);
...
test "built-in MCP runtime loads disabled configured servers without spawning" {
    ...
    try writeTempFile(&tmp, "home/.x1/mcp.json",
...
test "saving MCP config refuses a symlinked target" {
    ...
    try tmp.dir.symLink(io_mod.getIo(), external_path, "home/.x1/mcp.json", .{ .is_directory = false });

    const path = try std.fs.path.join(alloc, &.{ std.fs.path.dirname(external_path).?, ".x1", "mcp.json" });
    defer alloc.free(path);
...
test "adding an MCP server creates the profile directory privately" {
    ...
    const dir_stat = try tmp.dir.statFile(io_mod.getIo(), "home/.x1", .{ .follow_symlinks = false });
    try std.testing.expectEqual(@as(u32, 0o700), dir_stat.permissions.toMode() & 0o777);
    const file_stat = try tmp.dir.statFile(io_mod.getIo(), "home/.x1/mcp.json", .{ .follow_symlinks = false });
...
test "MCP auth requires explicit browser confirmation and logout stays non-secret" {
    ...
    try expectLine(
        started,
        "Waiting for MCP authentication for 'remote'. You can continue using x1 while the browser flow completes.",
        false,
    );
...
test "remote config keeps credential references and OAuth policy without secrets" {
    ...
    try std.testing.expectEqualStrings("x1-client", auth.client_id.?);
...
test "profile config rejects a mixed set containing invalid client metadata URLs" {
    const json =
        \\{"mcp":{"loopback":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"http://127.0.0.1:4321/client.json"}},"pathless":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"https://client.example"}},"root":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"https://client.example/"}},"valid":{"type":"http","url":"https://api.example.com/mcp","oauth":{"client_metadata_url":"https://client.example/x1.json"}}}}
    ;
    try std.testing.expectError(
