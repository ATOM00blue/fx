...
const KEEP_LARGE_SKILL_ARTIFACTS =
  process.env.X1_TUI_RESIZE_KEEP_ARTIFACTS === "1";
...
async function createResizeSession(
  ...
  let home = env.HOME;
  if (!home) {
    const root = mkdtempSync(join(tmpdir(), "x1-resize-home-"));
    tempDirs.push(root);
    home = join(root, "home");
    env.HOME = home;
  }
...
function createStderrPath(label: string): string {
  const dir = mkdtempSync(join(tmpdir(), `x1-resize-${label}-`));
  tempDirs.push(dir);
...
async function launchRecordedSurfaceSession(
  ...
  const root = realpathSync(
    mkdtempSync(join(tmpdir(), `x1-footer-surface-${label}-`)),
  );
  ...
  const active = await createResizeSession({
    ...
    env: {
      ...
      X1_AUTO_UPGRADE: "0",
      X1_RECORD: join(root, "session.fxtape"),
      X1_RECORD_INPUT: "1",
      NO_COLOR: "1",
      ...extraEnv,
    },
...
async function waitForLiveScrollbackText(
  ...
  while (Date.now() < deadline) {
    ...
    if (status.dead) {
      throw new Error(
        `x1 exited with status ${status.status} while waiting for ${JSON.stringify(needle)}.\nScrollback:\n${last}`,
      );
    }
...
function matchingTestSessions(): string[] {
  const prefix = `x1-test-${process.pid}-`;
  try {
    return execFileSync(
      "tmux",
...
function createSkillFixture(
  ...
  const root = realpathSync(
    mkdtempSync(join(tmpdir(), `x1-resize-${rootLabel}-${attempt}-`)),
  );
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const skillsRoot = join(home, ".x1", "skills");
  ...
  const fixtures = Array.from({ length: count }, (_, index) => {
    ...
    const marker = `x1-gauntlet-${attemptName}-${suffix}`;
...
function createFileApprovalRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-resize-file-approval-")));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(join(home, ".x1"), { recursive: true });
  mkdirSync(workspace, { recursive: true });
  ...
  try {
    ...
    const dismissed = await s.waitForPane(
      (pane) => {
        ...
        return pane.includes("layerx1.com") &&
          !pane.includes("↑↓ Navigate") &&
          findFooter(grid) !== null;
      },
      5_000,
...
describe.skipIf(SKIP)("tui: resize", () => {
  test(
    ...
    async () => {
      ...
      const headerRow = grid.findIndex((line) => line.includes("┌"));
      expect(headerRow).toBe(0);
      expect(grid.some((line) => line.includes("X1"))).toBe(true);
      expect(grid.some((line) => line.includes("layerx1.com"))).toBe(true);
    },
    TIMEOUT,
  );
  ...
  test(
    ...
    async () => {
      session = await createResizeSession({ cmd: "sleep 120" });
      const dir = mkdtempSync(join(tmpdir(), "x1-tmux-gate-failure-"));
      tempDirs.push(dir);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-long-transcript-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        cmd: `sh -c ${quoteShellPath(
          `printf 'PRE_X1_MARKER_long_resize\\n'; exec ${quoteShellPath(X1_BIN)}`,
        )}`,
        cwd: workspace,
        env: {
          HOME: home,
          ...
          X1_AUTO_UPGRADE: "0",
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "frame_schedule,frame_diff,frame_commit,scroll,resize",
          NO_COLOR: "1",
        },
        width: 120,
        height: 34,
      ...
      expect(scrollback.match(/layerx1\.com/g)).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-live-command-scrollback-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      const prex1Marker = "PRE_X1_RESIZE_STREAM_MARKER";
      ...
      expect(scrollback).not.toContain(prex1Marker);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-retention-native-scrollback-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      ...
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-cancel-command-approval-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      const transcriptCopyCounts = (scrollback: string) => ({
        startup: countOccurrences(scrollback, "X1"),
        help: countOccurrences(scrollback, "layerx1.com"),
      ...
      const replay = JSON.parse(
        execFileSync(X1_BIN, ["replay", tapePath, "--json"], { encoding: "utf8" }),
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-thematic-rule-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-nested-blockquote-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          X1_AUTO_UPGRADE: "0",
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "resize,frame_schedule,scroll",
          NO_COLOR: "1",
        },
        width: 120,
        height: 40,
  ...
  test(
    ...
    async () => {
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`x1 resize artifacts:\n${roots.join("\n")}`);
        console.log(
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-gated-stream-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`x1 gated stream resize artifact:\n${root}`);
      }
    },
    60_000,
  );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-open-picker-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      ...
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`x1 open picker resize artifact:\n${root}`);
      }
    },
    60_000,
  );
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-short-picker-dismissal-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-wide-user-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          X1_RECORD_INPUT: "1",
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "render,resize,transcript,input,worker",
          TMUX: undefined,
        },
        width: 80,
        height: 24,
      ...
      expect(readFileSync(tapePath).byteLength).toBeGreaterThan(0);
      if (KEEP_LARGE_SKILL_ARTIFACTS) {
        console.log(`x1 wide-user resize artifact:\n${root}`);
      }
    },
    TIMEOUT,
  );
  ...
  test(
    ...
    async () => {
      ...
      const first = "X1_HARD_ROW_ALPHA";
      const second = "X1_HARD_ROW_BETA";
  ...
  test(
    ...
    async () => {
      ...
      const first = "X1_SOFT_START";
      const second = "X1_SOFT_END";
  ...
  test(
    ...
    async () => {
      ...
      const first = "X1_HARD_4_ROW_ALPHA";
      const second = "X1_HARD_4_ROW_BETA";
      session = await launchAt(80, 24, { stderrPath });
      await enterHardNewlineInput(session, first, second);
  ...
  test(
    ...
    async () => {
      ...
      const first = "X1_HARD_9_ROW_ALPHA";
      const second = "X1_HARD_9_ROW_BETA";
      session = await launchAt(80, 24, { stderrPath });
      await enterHardNewlineInput(session, first, second);
    ...
    async () => {
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          X1_AUTO_UPGRADE: "0",
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "frame_schedule",
          NO_COLOR: "1",
        },
        stderrPath,
        width: 80,
  ...
  test(
    ...
    async () => {
      ...
      await session.sendKeys("Escape");
      await session.waitForText("layerx1.com", 5_000);
      const restored = captureScrollback();
      expect(restored.match(/layerx1\.com/g)).toHaveLength(1);
      expect(restored.match(/\bX1\b/g)).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), "x1-resize-prepaint-input-")),
      );
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const stderrPath = join(root, "stderr.log");
      ...
      // Drift the terminal position without updating x1's shadow grid. Tmux
      ...
      execFileSync(X1_BIN, ["replay", tapePath, "--frames-dir", replayDir], {
        cwd: workspace,
        stdio: "pipe",
      });
      ...
      expect(visibleViewport).not.toContain(prex1Marker);
      expect(session.paneStatus()).toEqual({ dead: false, status: null });
      expect(session.isPaneAlive()).toBe(true);
      expectEmptyStderr(stderrPath);
    },
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "x1-resize-large-paste-"));
      tempDirs.push(dir);
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          X1_GATEWAY_CHAT_URL: gateway.chatUrl,
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "input,worker,resize",
        },
      });
      await session.waitForComposer(10_000);
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "x1-resize-cpr-paste-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "x1-resize-cpr-paste-late-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "x1-resize-probe-timeout-"));
      tempDirs.push(dir);
      const tracePath = join(dir, "trace.log");
      const stderrPath = join(dir, "stderr.log");
      const gateway = startFakeGateway([
  ...
  test(
    "settled resize clears pre-x1 scrollback in both startup modes",
    async () => {
      for (const startupScrollback of [true, false]) {
        const root = mkdtempSync(join(tmpdir(), "x1-resize-history-reset-"));
        const home = join(root, "home");
        const workspace = join(root, "workspace");
        const marker = `PRE_X1_MARKER_${startupScrollback ? "ON" : "OFF"}_6179`;
        mkdirSync(join(home, ".x1"), { recursive: true });
        mkdirSync(workspace, { recursive: true });
        writeFileSync(
          join(home, ".x1", "settings.json"),
        ...
        expect(scrollback.match(/layerx1\.com/g)).toHaveLength(1);
        expect(scrollback.match(/\bX1\b/g)).toHaveLength(1);
  ...
  test(
    ...
    async () => {
      const root = mkdtempSync(join(tmpdir(), "x1-theme-reset-replay-"));
      const home = join(root, "home");
      const workspace = join(root, "workspace");
      const tracePath = join(root, "trace.log");
      const stderrPath = join(root, "stderr.log");
      ...
      session = await createResizeSession({
        ...
        env: {
          ...
          X1_RECORD: tapePath,
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "theme,frame_schedule,frame_commit,resize",
        },
        stderrPath,
        width: 120,
        height: 40,
