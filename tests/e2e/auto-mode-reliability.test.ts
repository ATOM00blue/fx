...
function createIsolatedRoot(baseDir = tmpdir()): IsolatedRoot {
  const root = realpathSync(
    mkdtempSync(join(baseDir, "x1-auto-mode-reliability-e2e-")),
  );
...
describe("lean auto mode reliability", () => {
  ...
  test(
    ...
    async () => {
      ...
      const first = await runx1(["ask", "--quiet", "--json", firstPrompt], {
        cwd: root.workspace,
        env,
        timeoutMs: TIMEOUT,
      });
      expect(first.code).toBe(0);
      const sessionIds = readdirSync(join(root.home, ".x1", "sessions"), {
        withFileTypes: true,
      })
        .filter((entry) =>
          entry.isDirectory() &&
          existsSync(join(root.home, ".x1", "sessions", entry.name, "session.json"))
        )
  ...
  test(
    ...
    async () => {
      ...
      const env = {
        ...
        PATH: `${bin}:${process.env.PATH ?? "/usr/bin:/bin"}`,
        X1_MEDIA_FRAMES: frames,
        X1_MEDIA_RENDER: renderedVideo,
        X1_MEDIA_PYTHON: pythonMarker,
      };
      const successful = await runx1(
        [
          "ask",
          "--quiet",
          "--json",
    ...
    async () => {
      ...
      const sessionIds = readdirSync(join(root.home, ".x1", "sessions"), {
        ...
        .filter((entry) =>
          entry.isDirectory() &&
          existsSync(
            join(root.home, ".x1", "sessions", entry.name, "session.json"),
          )
        )
        .map((entry) => entry.name);
      expect(sessionIds).toHaveLength(1);
