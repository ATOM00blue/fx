...
import { X1_BIN, HAS_API_KEY, runx1 } from "../evals/eval-helpers";
import {
  FAKE_GATEWAY_MODEL,
...
const liveTmuxTest = test.skipIf(
  !HAS_TMUX || !HAS_API_KEY || process.env.X1_E2E_REAL_API !== "1",
);
...
const LIFECYCLE_CYCLES = boundedEnvInt(
  "X1_FILE_PICKER_LIFECYCLE_CYCLES",
  PART4_STRESS ? 100 : 25,
  PART4_STRESS ? 100 : 2,
  10_000,
);
const LIFECYCLE_MEASURED_CYCLES = boundedEnvInt(
  "X1_FILE_PICKER_MEASURED_CYCLES",
  PART4_STRESS ? 120 : 12,
  PART4_STRESS ? 100 : 2,
  1_000,
);
...
function stressCandidateCount(): number {
  const raw = process.env.X1_FILE_PICKER_PART4_CANDIDATE_COUNT;
  if (raw === undefined || raw === "0") return 0;
  const value = Number.parseInt(raw, 10);
  if (!Number.isSafeInteger(value) || value < 50_000 || value > 100_000) {
    throw new Error(
      "X1_FILE_PICKER_PART4_CANDIDATE_COUNT must be 0 or an integer in [50000, 100000]",
    );
  }
  return value;
}
...
async function startMockx1(
  ...
  session = await TmuxSession.create({
    ...
    env: mockx1Environment(current, gateway, extraEnv),
    width: 112,
    height: 32,
...
function mockx1Environment(
  ...
  return {
    ...
    X1_AUTO_UPGRADE: "0",
    X1_TRACE_LOG: current.tracePath,
    X1_TRACE_SCOPES: "input,core,prompt,gateway,resize",
    X1_RECORD: current.tapePath,
    X1_RECORD_INPUT: "1",
    ...extraEnv,
  };
}
...
async function replayTape(current: Fixture): Promise<void> {
  await session?.kill();
  session = null;
  const replay = await runx1(["replay", current.tapePath], {
    cwd: current.workspace,
...
describe("@ file picker", () => {
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-stable-refresh-");
      const artifactDir = process.env.X1_FILE_PICKER_LIFECYCLE_ARTIFACT_DIR;
      ...
      const active = await startMockx1(
        current,
      ...
      const profileReadyPath = process.env.X1_FILE_PICKER_PROFILE_READY;
      const profileGoPath = process.env.X1_FILE_PICKER_PROFILE_GO;
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-unmatched-submit-");
      const active = await startMockx1(current, [
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-boundaries-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-narrow-prefix-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-duplicate-basename-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-added-root-");
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-repaint-");
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-refresh-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-safety-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-typed-integration-");
      initGit(current.workspace);
      writeFileSync(
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-filesystem-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-owned-spacing-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-spacing-");
      initGit(current.workspace);
  ...
  tmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-resume-");
      initGit(current.workspace);
      writeFileSync(
      ...
      const saved = await runx1(["session", "--id", sessionId!, "--json"], {
        cwd: current.workspace,
        env: { HOME: current.home },
        timeoutMs: TIMEOUT,
      });
      ...
      for (const cycle of [1, 2] as const) {
        ...
        session = await TmuxSession.create({
          ...
          cwd: current.workspace,
          env: mockx1Environment(current, activeGateway, {
            X1_RECORD: join(current.root, `resume-${cycle}.fxtape`),
          }),
      ...
      const resumedSaved = await runx1(
  ...
  liveTmuxTest(
    ...
    async () => {
      const current = createFixture("x1-file-picker-live-");
      initGit(current.workspace);
      ...
      session = await TmuxSession.create({
        ...
        env: {
          ...
          X1_AUTO_UPGRADE: "0",
          X1_MODEL: process.env.X1_FILE_PICKER_LIVE_MODEL ?? "anthropic/claude-sonnet-4.6",
          X1_TRACE_LOG: current.tracePath,
          X1_TRACE_SCOPES: "input,prompt,gateway",
        },
        width: 112,
        height: 32,
        stderrPath: current.stderrPath,
