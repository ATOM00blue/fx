...
function sessionIdFromHome(root: IsolatedRoot): string {
  const sessions = join(root.home, ".x1", "sessions");
...
function latestTraceReportPath(root: IsolatedRoot): string {
  const reports = readdirSync(root.root)
    .filter((entry) => entry.startsWith("x1-trace-") && entry.endsWith(".md"))
    .map((entry) => {
...
function createIsolatedRoot(baseDir = tmpdir()): IsolatedRoot {
  const root = realpathSync(mkdtempSync(join(baseDir, "x1-command-permissions-e2e-")));
...
function gatewayEnv(
  ...
  return {
    ...
    X1_MODEL: MODEL,
    X1_AUTO_UPGRADE: "0",
    X1_DIRECT_SECRET: "must-not-be-inherited",
    NO_COLOR: "1",
    ...extra,
  };
}
...
async function launchPermissionResumeHarness(initialResponses: Response[]) {
  ...
  const initialSession = await TmuxSession.create({
    cmd: X1_BIN,
    cwd: root.workspace,
    env: gatewayEnv(root, initialGateway, { X1_PERMISSION_MODE: undefined }),
  ...
  return {
    ...
    async resume(responses: Response[]) {
      ...
      const session = await TmuxSession.create({
        ...
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, { X1_PERMISSION_MODE: undefined }),
...
function commandReplayFiles(root: IsolatedRoot): string[] {
  ...
  const result = Bun.spawnSync(
    ["find", sessions, "-type", "f", "-name", "x1-command-replay-*"],
...
function largeEffectfulCommand(marker: string) {
  const command = [
    ...
    `printf '%s\\n' X1_LARGE_RUN_COMMAND_DONE > ${marker}`,
  ].join("\n");
...
describe("effect-aware command permissions", () => {
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          X1_TRACE_SCOPES: "gateway,permission,session,tool",
        }),
        stderrPath,
        width: 100,
        height: 28,
      ...
      await activeSession.waitForText("Yes, and tell x1 what to do next", TIMEOUT);
      ...
      const events = readFileSync(
        join(root.home, ".x1", "sessions", sessionId, "events.jsonl"),
        "utf8",
      );
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          X1_PERMISSION_MODE: "yolo",
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "core,permission,tool",
        }),
        stderrPath,
        width: 120,
        height: 40,
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          X1_TRACE_LOG: join(root.root, "minimal-command-output-trace.log"),
          X1_TRACE_SCOPES: "core,agent,tool,session,command_output",
        }),
        stderrPath,
        width: 120,
        height: 36,
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          X1_PERMISSION_MODE: "yolo",
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "core,tool,session,command_output",
        }),
        stderrPath,
        width: 72,
        height: 30,
      ...
      const sessionId = sessionIdFromHome(root);
      const publicSession = await runx1(
        ["session", "--id", sessionId, "--json"],
        { cwd: root.workspace, env: { HOME: root.home } },
      );
      ...
      expect(publicSession.stdout).toContain("<command_output_handle>x1-command-replay-");
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        cmd: X1_BIN,
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, { X1_PERMISSION_MODE: "yolo" }),
        stderrPath,
        width: 90,
        height: 30,
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      installClipboardFixture(
        root,
        '#!/bin/sh\nfor arg in "$@"; do last="$arg"; done\nprintf "%s" "$last" > "$X1_TRACE_CLIPBOARD_OUTPUT"\n',
      );
      writeFileSync(stderrPath, "");
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          PATH: hostilePath(root),
          TMPDIR: root.root,
          X1_TRACE_CLIPBOARD_OUTPUT: clipboardPath,
        }),
        stderrPath,
        width: 120,
        height: 40,
      ...
      expect(escapes).not.toContain("layerx1.com/feedback");
      expect(escapes).not.toContain("github.com");
      const reportPath = latestTraceReportPath(root);
      const report = readFileSync(reportPath, "utf8");
      expect(report).toContain("# x1 trace");
  ...
  test.skipIf(!tmuxAvailable())(
    "TUI feedback opens x1.sh without creating a trace or touching the clipboard",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([]);
      ...
      installUrlOpenerFixture(
        root,
        '#!/bin/sh\nprintf "%s" "$1" > "$X1_FEEDBACK_OPEN_OUTPUT"\n',
      );
      installClipboardFixture(
        root,
        '#!/bin/sh\nprintf used > "$X1_FEEDBACK_CLIPBOARD_MARKER"\n',
      );
      writeFileSync(stderrPath, "");
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          PATH: hostilePath(root),
          TMPDIR: root.root,
          X1_FEEDBACK_OPEN_OUTPUT: openerPath,
          X1_FEEDBACK_CLIPBOARD_MARKER: clipboardMarker,
        }),
        stderrPath,
        width: 120,
        height: 40,
      ...
      await activeSession.waitForText("Opened https://layerx1.com/feedback.", TIMEOUT);

      expect(readFileSync(openerPath, "utf8")).toBe("https://layerx1.com/feedback");
      expect(existsSync(clipboardMarker)).toBe(false);
      expect(
        readdirSync(root.root).filter((entry) => entry.startsWith("x1-trace-")),
      ).toHaveLength(0);
      const escapes = await activeSession.capturePaneEscapes();
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      const binary = process.env.X1_COMMAND_SESSION_TEST_BIN ?? X1_BIN;
      ...
      for (const sandbox of sandboxModes) {
        ...
        activeSession = await TmuxSession.create({
          ...
          env: gatewayEnv(root, gateway, {
            ...
            X1_RECORD_INPUT: "1",
            X1_TRACE_LOG: tracePath,
            X1_TRACE_SCOPES: "agent,core,gateway,permission,session,tool,worker",
          }),
          width: 120,
          height: 40,
          minimumHistoryLines: 1_000,
        ...
        await activeSession.sendText(
          "export PS1='X1_OUTER_PROMPT> '; printf 'X1_OUTER_SHELL_READY\\n'",
        );
        await activeSession.waitForText("X1_OUTER_SHELL_READY", TIMEOUT);
        await activeSession.sendText(
        ...
        const baselinex1 = foregroundx1Row(ttyPath, binary);
        ...
        try {
          ...
          expect(fixture.sid).not.toBe(baselinex1.sid);
          ...
          foregroundx1Row(ttyPath, binary);
        ...
        expect(commandResult).toMatch(
          /<command_output_handle>x1-command-replay-[^<]+<\/command_output_handle>/,
        );
        expect(gateway.requests[1]!.body).not.toContain("\\u001e");
        expect(gateway.requests[1]!.body).not.toContain("\\u0006");
        expect(gateway.requests[1]!.body).not.toContain("\\u0000");
        expect(gateway.requests[1]!.body).not.toContain("X1_FOREGROUND_EXEC_FAILED");
        ...
        expect(scrollback).not.toContain("X1_FOREGROUND_EXEC_FAILED");
        ...
        expect(replay.stdout).not.toContain("X1_FOREGROUND_EXEC_FAILED");
      }
    },
    90_000,
  );
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      rmSync(
        join(root.home, ".x1", "sessions", sessionId, "resume-view.bin"),
        { force: true },
      );
      activeSession = await TmuxSession.create({
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          X1_PERMISSION_MODE: "auto",
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "permission,interrupt",
        }),
        stderrPath,
        width: 120,
        height: 40,
  ...
  test(
    "default x1 ask defaults missing permission mode to auto",
    async () => {
      const root = createIsolatedRoot();
      ...
      const result = await runx1(["ask", "Create the marker."], {
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, {
        }),
        timeoutMs: TIMEOUT,
  ...
  test(
    "x1 ask yolo returns repeated user-profile command results to the model",
    async () => {
      const root = createIsolatedRoot();
      ...
      const result = await runx1(["ask", "--yolo", "Run pwd until you can answer."], {
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, {
          PATH: hostilePath(root),
        }),
  ...
  test(
    "x1 ask yolo completes more than ten serial user-profile commands when unlimited",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        ...Array.from(
  ...
  test(
    "x1 ask condition-waits for a canonical child without shell polling",
    async () => {
      const root = createIsolatedRoot();
      ...
      const result = await runx1(["ask", "Create and inspect one child."], {
        cwd: root.workspace,
  ...
  test(
    "x1 ask pauses and resumes provider recovery for a child",
    async () => {
      const root = createIsolatedRoot();
      ...
      const result = await runx1(["ask", "Create and recover a failing child."], {
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, { PATH: hostilePath(root) }),
        timeoutMs: TIMEOUT,
      });
  ...
  test(
    "x1 ask runs repeated turns for a persistent canonical subagent",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask delivers periodic child notifications at the next available parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask delivers two ordered child messages at one parent boundary",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask resumes a 64 KiB child message through five bounded projections",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask permits a child to create a nested canonical child",
    async () => {
      const root = createIsolatedRoot();
      ...
      const result = await runx1(["ask", "Create one child that creates another child."], {
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, { PATH: hostilePath(root) }),
        timeoutMs: TIMEOUT,
      });
  ...
  test(
    "x1 ask nested child receives periodic grandchild delivery at the next available step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 advertises and executes the canonical subagent tool",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 delivers a child approval to the next same-turn parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 lets a child continue after repeated advisory cautions",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 delivers periodic child notifications at the next available parent step",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 delivers a 64 KiB child message in five bounded projections",
    async () => {
      const root = createIsolatedRoot();
    ...
    async () => {
      ...
      activeSession = await TmuxSession.create({
        ...
        env: gatewayEnv(root, gateway, {
          ...
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "scroll,frame_commit",
        }),
        stderrPath,
        width: 120,
        height: 36,
      ...
      expect(JSON.parse(readFileSync(join(root.home, ".x1", "settings.json"), "utf8")).permission_mode)
        .toBe("ask");
      expect(gateway.requests).toHaveLength(1);
      expect(activeSession.isAlive()).toBe(true);
      expect(readFileSync(stderrPath, "utf8")).toBe("");
  ...
  test(
    "x1 ask yolo executes pwd through the default user profile without an artifact",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask defaults missing permission mode to auto through the classifier",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask does not retry a malformed classifier completion and safely replans",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask returns one malformed classifier completion to the agent without execution",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask provider failure never executes or enters malformed recovery",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask SIGINT during classifier wait terminates before decision or execution",
    async () => {
      const root = createIsolatedRoot();
      ...
      const child = nodeSpawn(
        ...
        ["ask", "--quiet", "--json", "--no-save", "Run the classifier cancellation fixture."],
        {
          ...
          env: definedEnv(gatewayEnv(root, gateway, {
            X1_TRACE_LOG: tracePath,
            X1_TRACE_SCOPES: "permission,stream",
          })),
      ...
      try {
        ...
        const result = await Promise.race([
          ...
          Bun.sleep(2_000).then(() => {
            throw new Error("x1 did not exit on SIGINT while the classifier remained blocked");
          }),
        ]);
  ...
  test(
    "x1 ask automatic review receives the exact delegated command",
    async () => {
      const root = createIsolatedRoot();
  ...
  test.skipIf(!tmuxAvailable())(
    "x1 ask terminal automatic caution returns advice without prompting",
    async () => {
      const root = createIsolatedRoot();
      const marker = join(root.workspace, "x1-ask-prompt-approved.txt");
      const command = `printf approved > ${JSON.stringify(marker)}`;
      const gateway = startFakeGateway(
        [
          toolCall(command),
          finalText("x1 ask prompt complete"),
        ],
        { classifierDecision: "caution" },
      );
      const tracePath = join(root.root, "trace.log");
      ...
      activeSession = await TmuxSession.create({
        cmd: `${shellQuote(X1_BIN)} ask --auto --no-save ${shellQuote("Run the one-shot prompt fixture.")}`,
        cwd: root.workspace,
        env: gatewayEnv(root, gateway, {
          X1_TRACE_LOG: tracePath,
          X1_TRACE_SCOPES: "permission",
      ...
      const finalPane = await activeSession.waitForText("x1 ask prompt complete", TIMEOUT);
  ...
  test(
    "x1 ask and ACP send large automatic review packets before execution",
    async () => {
  ...
  test(
    "x1 ask projects hostile ls filenames through the default user profile",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask preserves quoted shell metacharacters through the user profile",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        toolCall("printf '%s' '<'"),
  ...
  test(
    "x1 ask keeps parser hardening cases approval-bearing",
    async () => {
      const commands = [
        "wc -c < input.txt",
        "printf x\r|wc -c",
  ...
  test(
    "x1 ask blocks approval-bearing commands before side effects",
    async () => {
      const root = createIsolatedRoot();
      const marker = join(root.workspace, "must-not-exist");
  ...
  test(
    "x1 ask blocks hostile git before any executable or repository access",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startFakeGateway([
        toolCall("git status"),
