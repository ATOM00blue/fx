...
function createIsolatedRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-vision-route-e2e-")));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  mkdirSync(join(home, ".x1"), { recursive: true });
  mkdirSync(workspace, { recursive: true });
  writeFileSync(join(home, ".x1", "settings.json"), JSON.stringify({ permission: {} }));
...
function writeLegacyZeroImageSession(
  ...
  const sessionDir = join(root.home, ".x1", "sessions", sessionId);
...
function parsex1ErrorJson(result: Awaited<ReturnType<typeof runx1>>) {
...
describe("Vision route fake Gateway", () => {
  test(
    "x1 ask rejects missing images before Gateway startup in text and JSON modes",
    async () => {
      const root = createIsolatedRoot();
      const gateway = startImageGateway([]);
      const missingPath = join(root.workspace, "missing-image.png");
      try {
        const textResult = await runx1(
        ...
        const json = parsex1ErrorJson(jsonResult);
  ...
  test(
    "x1 ask gates GLM images through Vision without leaking paths",
    async () => {
      const root = createIsolatedRoot();
  ...
  test(
    "x1 ask recovers when the model rejects the post-Vision prompt as assistant prefill",
    async () => {
      const root = createIsolatedRoot();
      const fixture = createScopedImageFixture(root);
  ...
  test(
    "x1 ask executes path-source Vision and cleans transient snapshots without failure telemetry",
    async () => {
      const root = createIsolatedRoot();
      const imagePath = join(root.workspace, "path-source.png");
      const tracePath = join(root.root, "trace.log");
      const snapshotsBefore = readdirSync("/tmp")
        .filter((name) => name.startsWith("x1-image-snapshots-"))
        .sort();
      ...
      for (const entry of cases) {
        ...
        try {
          ...
          for (const request of gateway.chatRequests) {
            ...
            expect(request.body).not.toContain("x1-image-snapshots");
            expect(request.body).not.toContain(".x1/sessions");
          }
        } finally {
          gateway.stop();
          rmSync(root.root, { recursive: true, force: true });
  ...
  test(
    "x1 ask preserves native image parts for Gemini",
    async () => {
      const root = createIsolatedRoot();
      const fixture = createScopedImageFixture(root);
  ...
  test(
    "x1 ask normalizes encoded-oversized native images on macOS and rejects elsewhere",
    async () => {
      const root = createIsolatedRoot();
      ...
      try {
        ...
        const errorJson = parsex1ErrorJson(jsonResult);
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const firstJson = parsex1Json(first);
        ...
        const resumedJson = parsex1Json(resumed);
    ...
    async () => {
      ...
      for (const entry of cases) {
        ...
        try {
          ...
          const savedJson = parsex1Json(saved);
          expect(savedJson.output).toContain(`saved ${entry.name}`);
          expect(gateway.chatRequests).toHaveLength(1);

          const imageDir = join(root.home, ".x1", "sessions", savedJson.session_id, "images");
          ...
          const rereadJson = parsex1Json(reread);
  ...
  test(
    "x1 ask applies image_adapter_output_bytes to Vision provider capture",
    async () => {
      const root = createIsolatedRoot();
      const fixture = createScopedImageFixture(root);
      const gateway = startImageGateway([
  ...
  test(
    ...
    async () => {
      ...
      try {
        ...
        const nativeJson = parsex1Json(native);
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      try {
        ...
        await session.waitForText("Would you like to allow this action?", TIMEOUT);
        await session.sendKeys("Tab");
        await session.waitForText("Yes, and tell x1 what to do next", TIMEOUT);
        ...
        const sessionsRoot = join(root.home, ".x1", "sessions");
