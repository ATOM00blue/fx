...
function makePaths(label: string): Paths {
  ...
  const root = realpathSync(mkdtempSync(join(tempRoot, `x1-r-${label}-`)));
  return {
    root,
    home: join(root, "home"),
...
async function seedRealSession(paths: Paths, config: Config): Promise<IndexedSummary> {
  ...
  const indexPath = join(paths.home, ".x1", "sessions", "index.json");
...
function installLargeCatalog(
  ...
  const sessionsRoot = join(paths.home, ".x1", "sessions");
  ...
  try {
    session = await TmuxSession.create({
      ...
      env: {
        ...
        X1_TRACE_LOG: paths.trace,
        X1_TRACE_SCOPES: "core,session",
      },
      stderrPath: paths.stderr,
      width: 112,
      height: 32,
    ...
    const report = {
      config,
      catalogBytes: statSync(join(paths.home, ".x1", "sessions", "index.json")).size,
      transitions: {
...
test.skipIf(!tmuxAvailable() || process.env.X1_RESUME_BRUTAL !== "1")(
...
test.skipIf(
  !tmuxAvailable() ||
    process.env.X1_RESUME_PROFILE !== "1" ||
    platform() !== "darwin" ||
...
test.skipIf(!tmuxAvailable() || process.env.X1_RESUME_50K !== "1")(
  "/resume survives a fifty-thousand-entry catalog and real fifty-thousand-line tool-heavy session",
  async () => {
    const profileSeconds = process.env.X1_RESUME_PROFILE === "1" &&
        platform() === "darwin" &&
        existsSync("/usr/bin/sample")
      ? 30
      : undefined;
