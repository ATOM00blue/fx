...
import { EVAL_MODEL, HAS_API_KEY, runx1 } from "../evals/eval-helpers";

const TIMEOUT = 20_000;
const MODEL = "openai/gpt-5";
const liveTest = test.skipIf(
  !HAS_API_KEY || process.env.X1_E2E_REAL_API !== "1",
);
...
function createIsolatedRoot() {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-file-paths-e2e-")));
...
describe("filesystem path handling", () => {
  ...
  liveTest(
    ...
    async () => {
      ...
      const content = `X1_LIVE_ADDED_ROOT_${Date.now()}`;
      const instructionSentinel = "X1_LIVE_ADDED_ROOT_INSTRUCTION_MUST_NOT_APPEAR";
        ...
        const result = await runx1(
          ...
          ],
          {
            ...
            env: {
              ...
              X1_MODEL: process.env.X1_WORKSPACE_ACCESS_LIVE_MODEL ?? EVAL_MODEL,
            },
            timeoutMs: 120_000,
          },
        );
  ...
  test(
    ...
    async () => {
      const root = createIsolatedRoot();
      try {
        const homeFile = join(root.home, "x1-path-fixture.txt");
        const externalFile = join(root.external, "x1-path-fixture.txt");
        ...
        const cases = [
          {
            id: "read_home_1",
            path: "~/x1-path-fixture.txt",
            canonical: homeFile,
            content: "HOME_FIXTURE_CONTENT",
          },
          {
            id: "read_relative_1",
            path: "../external/x1-path-fixture.txt",
      ...
      try {
        ...
        for (const scenario of classifiedScenarios) {
          ...
          try {
            ...
            const classifiedJson = parsex1Json(classified);
  ...
  test(
    "registered typed write and edit use one canonical x1 ask mutation path",
    async () => {
      const root = createIsolatedRoot();
      try {
    ...
    async () => {
      ...
      try {
        ...
        await runTerminalToolScenario({
          ...
          name: "read_file",
          input: { path: "~/x1-path-fixture.txt" },
          unsetHome: true,
