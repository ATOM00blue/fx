 ...
 * freshly built x1 binary in tmux, arms one observer checkpoint, releases one
...
function createFixtureRoot(autoPermissions = false) {
  const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-ui-observer-fixture-")));
...
function createArtifactDir(keep: boolean): string {
  const configured = process.env.X1_UI_OBSERVER_ARTIFACT_DIR;
  if (configured) {
    mkdirSync(configured, { recursive: true });
    return realpathSync(configured);
  }
  const root = mkdtempSync(join(tmpdir(), "x1-ui-observer-"));
...
function gatewayEnv(
  ...
  return {
    ...
    X1_AUTO_UPGRADE: "0",
    X1_UI_OBSERVE_DIR: artifactDir,
    X1_RECORD: join(artifactDir, "session.fxtp"),
    NO_COLOR: "1",
  };
}
...
async function run() {
  ...
  if (!existsSync(X1_BIN)) {
    throw new Error(`fresh x1 binary is missing: ${X1_BIN}\nRun: zig build`);
  }
  ...
  const preserveArtifacts = args.keep || process.env.X1_UI_OBSERVER_ARTIFACT_DIR !== undefined;
  try {
