...
- Did the freshly built x1 binary emit coherent terminal bytes?
- Did replaying those bytes through `x1 replay` produce the expected model?
- Did the real terminal-owned text/grid settle after each user-visible event?
- Did old shell scrollback stay outside x1-owned viewport rows?
...
X1_RECORD tape and x1 replay
  Byte-level recording of what x1 wrote plus replay through the built-in virtual terminal.
...
2. Print shell markers and wrapped lines before x1 starts.
3. Launch the freshly built `zig-out/bin/x1`.
...
```bash
cd /Users/example/Developer/x1/x1-worktree-rendering
zig build
cd tests/e2e
bun run render-lab -- --scenario same-shell-relaunch --runs 1 --out /private/tmp/x1-render-lab
```

Default test wrapper:

```bash
cd /Users/example/Developer/x1/x1-worktree-rendering/tests/e2e
...
```bash
X1_RENDER_LAB_NATIVE=1 bun run render-lab -- --scenario native-terminal-app-relaunch --runs 1 --out /private/tmp/x1-native
```

Ghostty and Warp use clipboard-based selected-text capture, so they require an extra explicit gate:

```bash
X1_RENDER_LAB_NATIVE=1 X1_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD=1 \
  bun run render-lab -- --scenario native-ghostty-relaunch --runs 1 --out /private/tmp/x1-native-ghostty
```

The Command-K scenario requires a second gate because it triggers real terminal clear-scrollback behavior:

```bash
X1_RENDER_LAB_NATIVE=1 X1_RENDER_LAB_NATIVE_COMMAND_K=1 \
  bun run render-lab -- --scenario native-terminal-app-command-k --runs 1 --out /private/tmp/x1-native-command-k
```
...
- `trace.log`: x1 debug trace for repaint, resize, footer, input, and related scopes.
- `render.fxtape`: byte-level replay tape produced by the freshly built x1 binary.
- `replay-summary.json`: structured `x1 replay` output.
...
- at most one active x1 logo block
...
- shell markers do not appear inside the x1-owned viewport band
...
x1 must not replay old shell rows into its active viewport.
...
7. If native fails and tmux passes, the issue is likely terminal-app behavior, terminal capture behavior, or a terminal-specific x1 code path such as `TERM_PROGRAM=Apple_Terminal`.
...
More than one x1 logo block is visible in the active viewport. Usually means old x1 startup output was replayed or not cleared correctly.
...
A shell marker was found inside the active x1 viewport band. This is the primary old-scrollback-replayed-as-x1-content failure.
...
- Command-K requires System Events and the explicit `X1_RENDER_LAB_NATIVE_COMMAND_K=1` gate.
...
- Requires `X1_RENDER_LAB_NATIVE_ALLOW_CLIPBOARD=1`.
...
bun test tui-render-lab.test.ts
bun run render-lab -- --scenario same-shell-relaunch --runs 1 --out /private/tmp/x1-render-lab
../../zig-out/bin/x1 status --json
git diff --check
```

Use the exact freshly built binary from this checkout. Do not run bare `x1`.
