...
function parsex1Json(result: { stdout: string; stderr: string; code: number | null }): x1Json {
  if (result.code !== 0) {
    throw new Error(`x1 exited ${result.code}\nstdout: ${result.stdout}\nstderr: ${result.stderr}`);
  }
  return JSON.parse(result.stdout.trim()) as x1Json;
}
...
describe("external file permissions", () => {
  test(
    "x1 ask --yolo bypasses a configured write denial without a classifier request",
    async () => {
      const root = createIsolatedRoot("x1-yolo-permissions-");
      try {
        ...
        const { gateway, result } = await runWithFakeGateway(
          ...
          [
            ...
            "--no-save",
            "--yolo",
            `Use only the write_file tool to create ${target} with exactly this content: X1_E2E_YOLO.`,
          ],
          [
        ...
        expect(result.stderr).toContain(
          "YOLO enabled: x1 permission checks disabled",
        );
        const output = parsex1Json(result);
        expect(
          output.tool_calls.some(
            (call) => call.name === "write_file" && call.status === "success",
          ),
        ).toBe(true);
        expect(readFileSync(target, "utf8")).toBe("X1_E2E_YOLO");
  ...
  test(
    "x1 ask reads external paths and exercises classifier and rule-gated writes",
    async () => {
      const root = createIsolatedRoot("x1-file-permissions-");
      try {
        ...
        const tracePath = join(root.root, "permission-trace.log");
        writeFileSync(readTarget, "X1_E2E_EXTERNAL_READ\n");
        writeFileSync(classifiedTarget, "before");
        writeFileSync(join(root.home, ".x1", "settings.json"), "{}");
        ...
        const { result: readResult } = await runWithFakeGateway(
          ...
          [
            ...
            fakeGatewayFinalText("X1_E2E_EXTERNAL_READ"),
          ],
        );
        const read = parsex1Json(readResult);
        expect(read.tool_calls).toContainEqual({ name: "read_file", status: "success" });
        expect(read.output).toContain("X1_E2E_EXTERNAL_READ");
        ...
        const { gateway: classifiedGateway, result: classifiedResult } =
          await runWithFakeGateway(
            ...
            [
              ...
              "--no-save",
              "--auto",
              `Use only the write_file tool to overwrite ${classifiedTarget} with exactly this content: X1_E2E_EXTERNAL_CLASSIFIED.`,
            ],
            [
              fakeGatewayToolCall("classified_write_1", "write_file", {
                path: classifiedTarget,
                content: "X1_E2E_EXTERNAL_CLASSIFIED",
              }),
        ...
        const classified = parsex1Json(classifiedResult);
        expect(classified.tool_calls).toContainEqual({ name: "write_file", status: "success" });
        expect(readFileSync(classifiedTarget, "utf-8")).toBe("X1_E2E_EXTERNAL_CLASSIFIED");
        ...
        const { gateway: allowedGateway, result: allowedResult } = await runWithFakeGateway(
          ...
          [
            ...
            "--no-save",
            "--auto",
            `Use only the write_file tool to create ${allowedTarget} with exactly this content: X1_E2E_EXTERNAL_ALLOWED.`,
          ],
          [
            fakeGatewayToolCall("allowed_write_1", "write_file", {
              path: allowedTarget,
              content: "X1_E2E_EXTERNAL_ALLOWED",
            }),
            fakeGatewayFinalText("allowed write complete"),
          ],
        );
        const allowed = parsex1Json(allowedResult);
        expect(allowed.tool_calls).toContainEqual({ name: "write_file", status: "success" });
        expect(readFileSync(allowedTarget, "utf-8")).toBe("X1_E2E_EXTERNAL_ALLOWED");
