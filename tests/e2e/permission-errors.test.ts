...
async function waitForPaneExit(
  ...
  throw new Error(
    `Timed out waiting for x1 ask to exit.\n${await session.captureFullScrollback()}`,
  );
}
...
async function runTtyPromptPermissionsCase(
  ...
  const root = createIsolatedRoot(
    `x1-${outputMode}-prompt-permissions-${decision}-`,
  );
  ...
  try {
    session = await TmuxSession.create({
      cmd: `${JSON.stringify(X1_BIN)} ask --${outputMode} --prompt-permissions --no-save "Run the exact ${outputMode} fixture." > ${JSON.stringify(stdoutPath)}`,
      cwd: root.workspace,
      env: permissionEnv(root.home, gateway),
      remainOnExit: true,
    });
    const prompt = await session.waitForText("Approve? [y/N]", TIMEOUT);
    expect(prompt).toContain("x1 wants to run:");
    ...
    if (outputMode === "json") {
      const json = JSON.parse(stdout) as x1Json;
...
describe("generic permission typed errors", () => {
  test(
    ...
    async () => {
      const root = createIsolatedRoot("x1-permission-error-");
      ...
      try {
        ...
        const result = await runx1(["ask", "--json", "--no-save", "--auto", "Run the denied command."], {
          cwd: root.workspace,
          env: {
            HOME: root.home,
            AI_GATEWAY_API_KEY: "permission-error-fake-key",
  ...
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      const root = createIsolatedRoot("x1-json-auto-prompt-permissions-");
      ...
      try {
        session = await TmuxSession.create({
          cmd: `${JSON.stringify(X1_BIN)} ask --auto --json --prompt-permissions --no-save "Run the advisory caution fixture." > ${JSON.stringify(stdoutPath)}`,
          cwd: root.workspace,
          env: permissionEnv(root.home, gateway),
          remainOnExit: true,
        });
  ...
  test(
    ...
    async () => {
      ...
      for (const testCase of cases) {
        const root = createIsolatedRoot(
          `x1-${testCase.mode}-${testCase.optIn ? "opt-in" : "default"}-non-tty-`,
        );
        ...
        try {
          ...
          if (testCase.mode === "json") {
            const json = JSON.parse(result.stdout) as x1Json & {
              error: string;
            };
