...
function createNotificationRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-notifications-")));
  const home = join(root, "home");
  const workspace = join(root, "workspace");
  const profile = join(home, ".x1");
...
function notificationEnv(
  ...
  return {
    ...
    X1_AUTO_UPGRADE: "0",
    // Sound behavior under test: skip the harness-wide X1_SOUND=0 default so
    // the fixture settings and platform default stay authoritative.
    X1_SOUND: undefined,
    X1_TRACE_LOG: tracePath,
    X1_TRACE_SCOPES: "hooks,notifications",
    NO_COLOR: "1",
  };
}
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    const settingsPath = join(fixture.home, ".x1", "settings.json");
    ...
    try {
      ...
      expect(trace).toContain("handler=x1.sound.turn_end");
...
test(
  "x1 ask keeps redirected stdout JSON and stderr byte-clean with notifications enabled",
  async () => {
    const fixture = createNotificationRoot();
    const gateway = startFakeGateway([
...
test.skipIf(!tmuxAvailable())(
  "x1 ask correlates permission attention and turn-end notifications",
  async () => {
    const fixture = createNotificationRoot();
    ...
    try {
      session = await TmuxSession.create({
        cmd: `${X1_BIN} ask --no-save "Try the prepared command."`,
        cwd: fixture.workspace,
        env: notificationEnv(fixture.home, gateway, tracePath),
        remainOnExit: true,
      });
...
test.skipIf(!tmuxAvailable())(
  ...
  async () => {
    ...
    try {
      ...
      expect(waitingTrace).toContain("handler=x1.sound.attention_required");
      expect(existsSync(marker)).toBe(false);
