...
built x1 binary. It is pinned to:
...
`package-lock.json` and installs the pinned official runner. Bun executes x1's
...
starting scenarios when the freshly built `zig-out/bin/x1` is missing.
...
`./zig-out/bin/x1`. It does not implement MCP messages or transport behavior.
