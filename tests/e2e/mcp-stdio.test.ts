...
function createRoot(
  ...
  const root = realpathSync(mkdtempSync(join(tmpdir(), `x1-mcp-${label}-`)));
  cleanupRoot = root;
  ...
  const command = options.recordLaunchAttempts
    ? [
      "/bin/sh",
      "-c",
      `printf '%s\\n' "$$" >> "$X1_MCP_LAUNCH_LOG"; exec "$X1_MCP_FIXTURE_RUNTIME" "$X1_MCP_FIXTURE_PATH"`,
    ]
      ...
      mcp: {
        fixture: {
          ...
          environment: {
            ...
            X1_MCP_LAUNCH_LOG: options.recordLaunchAttempts
              ? launchLogPath
              : undefined,
            X1_MCP_FIXTURE_RUNTIME: options.recordLaunchAttempts
              ? process.execPath
              : undefined,
            X1_MCP_FIXTURE_PATH: options.recordLaunchAttempts
              ? scriptPath
              : undefined,
            X1_MCP_PID_PATH: join(root, "mcp.pid"),
            X1_MCP_MODE: options.mode ?? "normal",
            X1_MCP_CRASH_MARKER: join(root, "mcp-crashed"),
            X1_MCP_RECOVERY_READY_PATH: join(root, "mcp-recovery-ready"),
            X1_MCP_INVALIDATION_RELEASE_PATH: invalidationReleasePath,
            X1_MCP_RECOVERED_TOOL_NAME: options.recoveredToolName,
            X1_MCP_EXPECT_ELICITATION: options.expectedElicitation,
            X1_MCP_ELICITATION_URL: options.elicitationUrl,
            X1_MCP_RESOURCES_SUBSCRIBE: options.resourcesSubscribe === false
              ? "0"
              : undefined,
            X1_MCP_RESOURCE_TTL_MS: options.resourceTtlMs?.toString(),
            X1_MCP_LEGACY_VERSION: options.legacyVersion,
            X1_MCP_LEGACY_DISCOVERY_VERSIONS:
              options.legacyDiscoveryVersions?.join(","),
            X1_MCP_LEGACY_DISCOVERY_METHOD_NOT_FOUND:
              options.legacyDiscoveryMethodNotFound ? "1" : undefined,
            X1_MCP_LEGACY_DISCOVERY_INVALID_PARAMS:
              options.legacyDiscoveryInvalidParams ? "1" : undefined,
            X1_MCP_LEGACY_REJECT_NEWER_INITIALIZE:
              options.legacyRejectNewerInitialize ? "1" : undefined,
            X1_MCP_DRAFT7_PATTERN: options.draft7Pattern,
            X1_MCP_URL_REQUIRED_OPERATION: options.urlRequiredOperation,
            X1_MCP_ENV_CAPTURE: options.captureEnvironment
              ? environmentCapturePath
              : undefined,
            X1_MCP_ENV_SENTINEL: options.captureEnvironment
              ? "configured"
              : undefined,
          },
  ...
  return {
    ...
    wireLogPath,
    launchLogPath,
    traceLogPath: join(root, "x1-trace.log"),
...
function fixtureEnv(root: FixtureRoot, activeGateway: ReturnType<typeof startFakeGateway>) {
  return {
    ...
    X1_GATEWAY_BASE_URL: activeGateway.baseUrl,
    X1_GATEWAY_CHAT_URL: activeGateway.chatUrl,
    X1_E2E_GATEWAY_CHAT_URL: activeGateway.chatUrl,
    X1_MODEL: MODEL,
    X1_TRACE_LOG: root.traceLogPath,
    X1_TRACE_SCOPES: "mcp",
  };
}
...
describe("modern MCP stdio compatibility", () => {
  test.skipIf(!tmuxAvailable())(
    ...
    async () => {
      ...
      expect(approval).toContain(
        "This MCP tool needs approval before x1 can send the request.",
      );
  ...
  test("repository-local MCP configuration never launches a process or network request", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-mcp-project-trust-")));
    cleanupRoot = root;
    const home = join(root, "home");
    const workspace = join(root, "workspace");
    const marker = join(root, "project-mcp-launched");
    mkdirSync(join(home, ".x1"), { recursive: true });
    mkdirSync(join(workspace, ".x1"), { recursive: true });
    writeFileSync(join(home, ".x1", "settings.json"), JSON.stringify({}));
    writeFileSync(join(home, ".x1", "mcp.json"), JSON.stringify({ mcp: {} }));
    ...
    writeFileSync(join(workspace, ".x1", "mcp.json"), hostile);
  ...
  test("fresh and resumed native sessions reconstruct MCP from the current profile", async () => {
    ...
    profile.mcp.fixture.environment.X1_MCP_INITIAL_TOOL_NAME = "sum";
    profile.mcp.fixture.environment.X1_MCP_RESULT_TEXT = "RESUMED_PROFILE_TOOL_RESULT";
  ...
  for (const childMode of ["one_off", "persistent"] as const) {
    ...
    test(`${label} child with no configured MCP runtime fails closed before transport`, async () => {
      ...
      writeFileSync(join(root.home, ".x1", "mcp.json"), JSON.stringify({ mcp: {} }));
    ...
    test(`${label} scoped search refreshes only its admitted stale MCP server`, async () => {
      ...
      profile.mcp = {
        allowed: {
          ...
          environment: {
            ...base.environment,
            X1_MCP_WIRE_LOG: allowedWirePath,
            X1_MCP_PID_PATH: join(root.root, "allowed.pid"),
            X1_MCP_INITIAL_TOOL_NAME: "echo",
            X1_MCP_RECOVERED_TOOL_NAME: "echo",
          },
        },
        denied: {
          ...base,
          environment: {
            ...base.environment,
            X1_MCP_WIRE_LOG: deniedWirePath,
            X1_MCP_PID_PATH: join(root.root, "denied.pid"),
            X1_MCP_INITIAL_TOOL_NAME: "blocked",
            X1_MCP_RECOVERED_TOOL_NAME: "blocked",
          },
        },
      };
      writeFileSync(profilePath, JSON.stringify(profile));
  ...
  test("x1 ask uses typed Resources Prompts and Completion flows", async () => {
  ...
  test("x1 ask cancels a bounded stalled resource read", async () => {
  ...
  test("a blocked write recovers before the next unsent runtime call", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-mcp-runtime-recovery-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  test("catalog waits preserve operation timeout and cancellation", async () => {
    for (const control of ["timeout", "cancel"] as const) {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), `x1-mcp-catalog-${control}-`)),
      );
      cleanupRoot = root;
      const proc = Bun.spawn(
        [
  ...
  test("stalled recovery preserves startup timeout and local cancellation", async () => {
    for (const control of ["timeout", "cancel"] as const) {
      const root = realpathSync(
        mkdtempSync(join(tmpdir(), `x1-mcp-recovery-${control}-`)),
      );
      cleanupRoot = root;
      const proc = Bun.spawn(
        [
  ...
  test("concurrent server recovery preserves whole-runtime tool-name uniqueness", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-mcp-recovery-collision-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  test("a failed reconnect leaves the remaining restart budget reachable", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-mcp-recovery-budget-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  test("a queued stale tool cannot cross a recovered connection generation", async () => {
    const root = realpathSync(mkdtempSync(join(tmpdir(), "x1-mcp-stale-recovery-")));
    cleanupRoot = root;
    const proc = Bun.spawn(
      [
        "zig",
  ...
  for (
    ...
    test(`x1 ask calls the ${fixture.label} stdio fixture`, async () => {
    ...
    const result = await runx1(
      ["ask", "--json", "--auto", "--no-save", "Call the environment MCP fixture."],
      {
        ...
        env: {
          ...fixtureEnv(root, activeGateway),
          PATH: parentPath,
          X1_MCP_INHERITED_SENTINEL: inheritedSentinel,
  ...
  test("x1 ask does not start an unused optional MCP server", async () => {
  ...
  test("x1 ask accepts the official legacy SDK Draft 7 tool schema", async () => {
  ...
  test("x1 ask rejects invalid legacy Draft 7 arguments before transport", async () => {
  ...
  test("x1 ask routes legacy stdio progress", async () => {
  ...
  test("noninteractive x1 ask returns typed input-required without fabricating a continuation", async () => {
  ...
  for (const surface of ["TUI", "Ask"] as const) {
    test.skipIf(!tmuxAvailable())(
      ...
      async () => {
        ...
        gateway = activeGateway;
        const stderrPath = join(root.root, "stderr.log");
        const binary = join(REPO_ROOT, "zig-out", "bin", "x1");
        tui = await TmuxSession.create({
          isolated: true,
          ...(surface === "Ask"
            ? {
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 ask validates and submits a modern MCP form elicitation",
    async () => {
  ...
  for (const legacyVersion of ["2025-06-18", "2025-11-25"] as const) {
    test.skipIf(!tmuxAvailable())(
      `interactive x1 ask handles negotiated ${legacyVersion} direct elicitation/create`,
      async () => {
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 ask validates Unicode patterns, edits, and submits every form field kind",
    async () => {
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 ask retries a URL browser failure without prefetching",
    async () => {
      let targetRequests = 0;
      const target = Bun.serve({
        port: 0,
      ...
      writeFakeUrlOpeners(
        fakeBin,
        "#!/bin/sh\nprintf '%s\\n' \"$1\" >> \"$X1_E2E_OPEN_LOG\"\nif [ ! -e \"$X1_E2E_OPEN_STATE\" ]; then touch \"$X1_E2E_OPEN_STATE\"; exit 1; fi\nexit 0\n",
      );
  ...
  test.skipIf(!tmuxAvailable())(
    "interactive x1 ask can refuse a URL without launching a browser",
    async () => {
      let targetRequests = 0;
      const target = Bun.serve({
        port: 0,
  ...
  test("x1 ask routes progress and times out a stalled operation without leaking its child", async () => {
    ...
    const progressResult = await runx1(
    ...
    const timeoutResult = await runx1(
  ...
  test("x1 ask bounds startup timeouts and reaps every attempted child", async () => {
  ...
  test("required profile startup failure blocks x1 ask before any Gateway request", async () => {
  ...
  test.skipIf(process.platform === "win32" || !tmuxAvailable())(
    ...
    async () => {
      ...
      writeFileSync(join(root.home, ".x1", "mcp.json"), "{not valid json");
  ...
  test.skipIf(process.platform === "win32" || !tmuxAvailable())(
    ...
    async () => {
      ...
      const profilePath = join(root.home, ".x1", "mcp.json");
      const profile = JSON.parse(readFileSync(profilePath, "utf8"));
      profile.mcp.fixture.environment.X1_MCP_MODE = "stall_startup";
      ...
      profile.mcp.fixture.environment.X1_MCP_MODE = "normal";
      writeFileSync(profilePath, JSON.stringify(profile));
  ...
  test.skipIf(process.platform === "win32" || !tmuxAvailable())(
    ...
    async () => {
      ...
      profile.mcp.fixture.environment.X1_MCP_MODE = "normal";
      profile.mcp.fixture.environment.X1_MCP_INITIAL_TOOL_NAME = "sum";
      profile.mcp.fixture.environment.X1_MCP_RESULT_TEXT = "RELOADED_TOOL_RESULT";
      writeFileSync(profilePath, JSON.stringify(profile));
  ...
  test("x1 ask performs one fresh-discovery restart without replaying the failed call", async () => {
  ...
  for (const childMode of ["one_off", "persistent"] as const) {
    test(`revoked ${childMode} authority prevents stdio recovery effects`, async () => {
      const root = realpathSync(mkdtempSync(join(tmpdir(), `x1-mcp-${childMode}-recovery-`)));
      cleanupRoot = root;
      const proc = Bun.spawn(
        [
          "zig",
  ...
  test("x1 ask stops restarting after the configured stdio budget", async () => {
