...
async function startx1(
  ...
  root = realpathSync(mkdtempSync(join(tmpdir(), "x1-edit-contracts-")));
  ...
  const skillRoot = join(home, ".x1", "skills", "review");
  ...
  session = await TmuxSession.create({
    ...
    env: {
      ...
      X1_E2E_GATEWAY_MODELS_URL: gateway
        ? `${gateway.baseUrl}/coding-agent/v1/models`
        : undefined,
      X1_MODEL: withGateway ? FAKE_GATEWAY_MODEL : undefined,
      X1_AUTO_UPGRADE: "0",
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(true, 2);
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(true);
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(true, 1, false, "input,theme,resize,native_clear");
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(true, 1, false, "input");
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(false);
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(true, 1, true);
...
for (
  ...
  tmuxTest(
    ...
    async () => {
      const active = await startx1(true, 1, false, "prompt_history");
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(true, 2, true);
...
tmuxTest(
  ...
  async () => {
    const active = await startx1(true, 4);
