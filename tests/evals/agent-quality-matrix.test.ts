...
const INITIAL_OR_RUNNABLE_PROMPTS = [
  ...
  "Look for changes/last commits in x1.",
  ...
  "Read https://github.com/vercel-labs/x1/pull/57 comments.",
...
describe("agent quality baseline matrix", () => {
  ...
  test("covers ask-user and blocker cases", () => {
    ...
    expect(firstToolMatchesExpectation(ghBlockerRow!, {
      name: "terminal",
      command_result: { command: "gh pr view 57 --repo vercel-labs/x1 --comments" },
    })).toBe(true);
    expect(firstToolMatchesExpectation(ghBlockerRow!, {
      name: "terminal",
    ...
    expect(releaseBumpChoiceBlockerSurfaced(
      "x1 ask: repeated identical tool call requires approval",
    )).toBe(false);
  });
