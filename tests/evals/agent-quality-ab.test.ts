...
describe("agent quality A/B harness helpers", () => {
  ...
  test("rejects bare x1 and relative binary paths", () => {
    expect(() => requireAbsoluteExecutableBinary("x1", "baseline")).toThrow(/absolute path/);
    expect(() => requireAbsoluteExecutableBinary("./zig-out/bin/x1", "candidate")).toThrow(/absolute path/);
  });

  test("redacts credential-looking values", () => {
    expect(redactSensitiveValue("AI_GATEWAY_API_KEY", "secret-value")).toBe("[redacted]");
    expect(redactSensitiveValue("X1_MODEL", "provider/test-model")).toBe("provider/test-model");
  });
...
const hasLiveAbConfig = Boolean(
  process.env.X1_AB_BASELINE_BIN &&
    process.env.X1_AB_CANDIDATE_BIN &&
    process.env.X1_AB_MODEL &&
