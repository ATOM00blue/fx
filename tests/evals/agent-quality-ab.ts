 ...
 *
 * This compares two explicit x1 binaries on the same focused matrix rows. It is
...
export function loadAbConfigFromEnv(env: NodeJS.ProcessEnv = process.env): AbConfig {
  const baselineBin = requireAbsoluteExecutableBinary(
    env.X1_AB_BASELINE_BIN ?? "",
    "baseline",
  );
  const candidateBin = requireAbsoluteExecutableBinary(
    env.X1_AB_CANDIDATE_BIN ?? "",
    "candidate",
  );
  const model = env.X1_AB_MODEL;
  if (!model) throw new Error("X1_AB_MODEL is required for A/B runs");

  const trials = Number(env.X1_AB_TRIALS ?? "3");
  if (!Number.isInteger(trials) || trials <= 0) {
    throw new Error(`X1_AB_TRIALS must be a positive integer, got ${env.X1_AB_TRIALS}`);
  }

  const outputDir =
    env.X1_AB_OUTPUT_DIR ??
    mkdtempSync(join(tmpdir(), `x1-agent-quality-ab-${Date.now()}-`));

  return {
    baselineBin,
    candidateBin,
    model,
    rowIds: parseRowIds(env.X1_AB_ROWS),
    trials,
    outputDir,
    workspaceRoot: env.X1_AB_WORKSPACE_ROOT ?? REPO_ROOT,
    timeoutMs: Number(env.X1_AB_TIMEOUT_MS ?? "300000"),
  };
}
...
function sanitizedEnvMetadata(model: string): Record<string, string> {
  const keys = ["X1_MODEL", "AI_GATEWAY_API_KEY", "VERCEL_OIDC_TOKEN", "NO_COLOR"];
  const metadata: Record<string, string> = {};
  for (const key of keys) {
    const value = key === "X1_MODEL" ? model : process.env[key];
...
export async function runAbTrial(
  ...
  const trialHome = mkdtempSync(join(tmpdir(), `x1-ab-${row.id}-${trialIndex}-${side}-`));
  const env: Record<string, string | undefined> = {
    PATH: process.env.PATH ?? "",
    HOME: trialHome,
    NO_COLOR: "1",
    X1_MODEL: config.model,
  ...
  try {
    ...
    if (json.model && json.model !== config.model) {
      score = {
        ...score,
        passed: false,
        reason: `${score.reason}; json.model ${json.model} did not match X1_AB_MODEL ${config.model}`,
      };
    }
  } catch (err) {
    score = {
